...
# 발견 섹션 헤더(RENDER_HEADER_*) **바로 앞**까지 — 헤더가 없으면(발견 0) 문서 전체다. 그 뒤는
...
# 형식 위반**이다. 값 **앞**의 같은 줄 텍스트만 허용한다 — 실호출에서 모델이 총평 문단 끝에
# 계약 줄을 이어 붙이는 형태가 실제로 나왔고(#283 probe3), 이건 계약을 어긴 게 아니라 줄바꿈
# 하나 차이다. 앞을 허용해도 (가)"문서 어디든" 의 인용 오탐은 생기지 않는다: 구역의 **마지막
# 한 줄**만 보므로, 리뷰어가 계약문을 인용하면 그 인용의 꼬리가 채택돼 미산출로 접힌다.
# 계약 줄 뒤 산문은 곧 미산출이므로, 구 `LEGACY_UNABLE` 산문 휴리스틱이 계약 경로에 없어도
# 그 문구를 단 응답은 여기서 형식으로 접힌다.
if [ "$STATUS_CONTRACT" = 1 ]; then
  last_line=$(awk -v h1="$RENDER_HEADER_ONE" -v hn="$RENDER_HEADER_MANY" '
    { line[NR] = $0 }
    END {
      tail = NR                                               # 모델 통제 구역의 끝(기본: 문서 끝)
      for (i = 1; i <= NR; i++) {
        s = line[i]
        sub(/^[ \t]+/, "", s); sub(/[ \t]+$/, "", s)
        if (s == h1 || s == hn) { tail = i - 1; break }        # 첫 헤더 앞에서 끊는다 — 뒤는 codex 렌더
      }
      for (i = tail; i >= 1; i--) {
        s = line[i]
        sub(/^[ \t]+/, "", s); sub(/[ \t]+$/, "", s)
