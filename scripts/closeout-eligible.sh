...
printf '%s' "$prs" | jq -c '.[]' | while IFS= read -r row; do
  ...
  # ✅ 본문의 `코멘트 스냅샷 N` 워터마크 — verify-runner 가 사람 코멘트를 **읽은 시점**의
  ...
  #
  # 워터마크가 없는 **옛 ✅**(토큰 도입 이전·확인 단계 이전에 찍힌 것)은 종전 경계
  ...
  # 경계는 워터마크가 있어도 **절대 `vi+1` 을 넘지 않는다**(= min(N, vi+1)). N ≤ vi 는
  ...
  # 스크립트가 지키는 방향(증명 못 하면 통과 아님)의 정반대라 상한을 박아 둔다. 워터마크가
  ...
  # 실제로 쓰는 경계값은 `$cut` 이다 — ✅ 본문에 `코멘트 스냅샷 N` 워터마크가 있으면 N,
  # 없으면 `vi+1`(위 워터마크 자리 주석). 워터마크가 있는 ✅ 에선 verify-runner 의
