...
#     뮤테이션: 긍정 게이트 jq(`$vi` 계산)의 `| last` 를 `first` 로 바꾸면 $vi 가 구판정
#     (인덱스 0)을 가리켜 사이의 무마커 코멘트를 다시 세고, 이 케이스가 **탈락으로 빨개진다**.
...
# 22) 영문 판정 `Merge verdict: ✅` 가 경계인 형상 — ✅ 앞 무마커 사람 코멘트 → 후보.
#     경계 술어는 한/영 startswith 둘의 OR 다. 16~21 은 전부 한글 ✅ 라 영문 갈래가 죽어도
#     초록이다. 뮤테이션: `$vi` jq 에서 `or startswith("Merge verdict: ✅")` 를 지우면 $vi 가
#     비어 게이트에서 탈락 → 이 케이스가 **빨개진다**.
run_case "영문 ✅ 경계·앞 무마커→후보(#379)" yes '[
  {"body":"Rebase note: merged #5103 into this branch","createdAt":"2026-07-05T04:15:00Z"},
  {"body":"Merge verdict: ✅ ready to merge\n<!-- bodat:worker -->","createdAt":"2026-07-05T04:20:00Z"}
]' "2026-07-05T04:10:00Z"

# ── #384(Codex 2회차): 경계는 ✅ 본문의 `코멘트 스냅샷 N` 워터마크다 ──────────
# verify-runner ④ 의 사람 코멘트 확인 단계는 코멘트를 **읽고 나서** ✅ 를 게시한다 —
# 그 사이(수 초)에 끼어든 사람 코멘트는 ✅ 보다 **앞** 인덱스에 앉아, 경계가 ✅ 인덱스
# 였을 때는 "검증자가 이미 확인한 것" 으로 넘어갔다(경합 창). 이제 verify-runner 가
# 읽은 시점의 전체 코멘트 수 N 을 ✅ 본문에 적고 이 필터는 **인덱스 ≥ N** 을 센다.
# 아래 23·24 가 그 경계를 양방향으로 고정하고, 워터마크 없는 옛 ✅ 는 16·20·21·22 가
# 종전 경계(✅ 인덱스) 그대로 지킨다 — 소급 적용하지 않기로 한 결정(#379)의 회귀망.

# 23) **핵심 뮤테이션 대상** — 워터마크 + 경합: 스냅샷 1 은 "인덱스 0 까지만 읽었다" 는
#     뜻인데 인덱스 1 에 사람 코멘트가 끼었다(읽기~게시 사이) → 세어져 **탈락**.
#     뮤테이션: cutoff 를 워터마크 무시하고 항상 `vi+1`(= 종전 `.key > $vi`)로 되돌리면
#     인덱스 1 이 ✅ 앞이라 안 세어지고 **후보로 뜨며 빨개진다**(경합 창 재현).
run_case "워터마크·읽기~게시 사이 사람 코멘트→탈락(#384)" no '[
  {"body":"머지 판정: 🔄 진행 중\n<!-- bodat:worker -->","createdAt":"2026-07-05T04:00:00Z"},
  {"body":"이 부분 다시 봐 주세요","createdAt":"2026-07-05T04:19:00Z"},
  {"body":"머지 판정: ✅ 머지 가능 — 결정적 CI pass · 미해결 없음 · 코멘트 스냅샷 1\n<!-- bodat:worker -->","createdAt":"2026-07-05T04:20:00Z"}
]' "2026-07-05T04:10:00Z"
#     탈락은 17 과 같은 `blocked:` 한 줄로 드러나야 한다 — 경합 창이라고 조용히 죽이지
#     않는다(조용한 큐 사망 금지, SKILL #206 원칙).
if [ "$(grep -c '^blocked:.*#5.*1건' "$tmp/last.err")" = 1 ]; then
  pass=$((pass + 1))
else
  fail=$((fail + 1))
  echo "  ✗ 워터마크 탈락 노출(#384) — stderr 에 'blocked: … #5 … 1건' 한 줄이 있어야 한다:"
  sed 's/^/      /' "$tmp/last.err"
fi

# 24) 워터마크 정상 형상 — 스냅샷 1 이고 무마커 사람 코멘트는 인덱스 0(< 1) 이라
#     verify-runner 가 **읽고 확인한** 것 → 후보. 23 의 역방향: 워터마크를 "전부 다시
#     세라"(cutoff 0)로 읽으면 이 케이스가 탈락으로 빨개진다. 워터마크 도입이 정상
#     후보를 막지 않는다는 확인이기도 하다.
run_case "워터마크·확인된 ✅앞 코멘트→후보(#384)" yes '[
  {"body":"사람 세션 재심(policy 해제): 사용자 결정 — 이 PR 에서 닫는다.","createdAt":"2026-07-05T04:05:00Z"},
  {"body":"머지 판정: ✅ 머지 가능 — 결정적 CI pass · 사람 코멘트 1건 확인 · 코멘트 스냅샷 1\n<!-- bodat:worker -->","createdAt":"2026-07-05T04:20:00Z"}
]' "2026-07-05T04:10:00Z"

# 25) 워터마크가 ✅ 인덱스보다 **큰** 형상(스냅샷 5 인데 ✅ 는 인덱스 0) → 탈락.
#     N ≤ vi 는 정상 경로에선 구조적으로 보장되지만(읽고 나서 게시), ㉮ verify-runner 가
#     수를 잘못 적거나 ㉯ ✅ 이후 코멘트가 **삭제**돼 배열이 줄면 깨진다 — 그때 경계가
#     ✅ 뒤로 밀려 **✅ 이후의 사람 리뷰를 건너뛰는 fail-open** 이 된다. 경계는 워터마크가
#     있어도 절대 `vi+1` 을 넘지 않는다(= cutoff 는 min(N, vi+1)).
#     뮤테이션: SUT 의 `[ "$cut" -le "$vi" ] || cut=$((vi + 1))` 를 지우면 cutoff 가 5 가
#     되어 인덱스 1 의 사람 코멘트를 안 세고 **후보로 뜨며 빨개진다**.
run_case "워터마크 N > ✅ 인덱스(삭제·오기)→탈락(#384)" no '[
