...
# ── #384(Codex 2회차): 경계는 ✅ 본문의 `코멘트 스냅샷 N` 토큰이다 ──────────
...
# 아래 23·24 가 그 경계를 양방향으로 고정하고, 스냅샷 토큰 없는 옛 ✅ 는 16·20·21·22 가
# 종전 경계(✅ 인덱스) 그대로 지킨다 — 소급 적용하지 않기로 한 결정(#379)의 회귀망.

# 23) **핵심 뮤테이션 대상** — 스냅샷 + 경합: 스냅샷 1 은 "인덱스 0 까지만 읽었다" 는
#     뜻인데 인덱스 1 에 사람 코멘트가 끼었다(읽기~게시 사이) → 세어져 **탈락**.
#     뮤테이션: cutoff 를 스냅샷을 무시하고 항상 `vi+1`(= 종전 `.key > $vi`)로 되돌리면
#     인덱스 1 이 ✅ 앞이라 안 세어지고 **후보로 뜨며 빨개진다**(경합 창 재현).
run_case "스냅샷·읽기~게시 사이 사람 코멘트→탈락(#384)" no '[
  {"body":"머지 판정: 🔄 진행 중\n<!-- bodat:worker -->","createdAt":"2026-07-05T04:00:00Z"},
...
if [ "$(grep -c '^blocked:.*#5.*1건' "$tmp/last.err")" = 1 ]; then
  ...
  fail=$((fail + 1))
  echo "  ✗ 스냅샷 탈락 노출(#384) — stderr 에 'blocked: … #5 … 1건' 한 줄이 있어야 한다:"
  sed 's/^/      /' "$tmp/last.err"
fi

# 24) 스냅샷 정상 형상 — 스냅샷 1 이고 무마커 사람 코멘트는 인덱스 0(< 1) 이라
#     verify-runner 가 **읽고 확인한** 것 → 후보. 23 의 역방향: 스냅샷을 "전부 다시
#     세라"(cutoff 0)로 읽으면 이 케이스가 탈락으로 빨개진다. 스냅샷 도입이 정상
#     후보를 막지 않는다는 확인이기도 하다.
run_case "스냅샷·확인된 ✅앞 코멘트→후보(#384)" yes '[
...
run_case "스냅샷 N > ✅ 인덱스(삭제·오기)→탈락(#384)" no '[
