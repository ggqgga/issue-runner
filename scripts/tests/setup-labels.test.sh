...
# ⑵ 라벨 세트가 전부 만들어졌다. 기대치는 스크립트가 정의한 `gh label create` 줄 수 —
#    라벨을 더하거나 빼면 함께 움직이되, 중간에 멎으면 어긋난다.
defined=$(grep -c '^gh label create ' "$SUT")
{ [ "$created" = "$defined" ] && [ "$created" -ge 20 ]; } && ok \
  || bad "⑵ 라벨 생성 $created 건 (정의 $defined 건) — 중간에 멎었거나 세트가 20건 미만이다"
