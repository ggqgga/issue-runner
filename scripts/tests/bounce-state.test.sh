...
#        · **해소 보고 표식**(`완료`·`해소` — 부정 접두 `미`/`불` 이 붙거나 뒤에 요구
#          어휘(`요망`·`필요`·`바람`·`주세요` …)가 붙으면 제외)이 꼬리에 있으면
#          → `ok`. 반송이 아니라 **반송을 푼 보고**다(#251 ② — 원장 실측 4건).
#          요구 어휘 거부는 사전 리뷰가 잡은 칸이다 — `재디스패치 요청 — BLOCKER 해소
#          요망` 은 해소 **보고**가 아니라 해소를 **요구하는 반송**이라 `bounced` 다.
