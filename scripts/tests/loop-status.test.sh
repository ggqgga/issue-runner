...
#                          **의도된 잔여**(#371 ⓑ)를 못 박는 칸이다: 각 200건 상한 절단으로
#                          둘 중 어느 목록에도 없는 번호는 "못 봤다" 이므로 조용한 쪽으로
#                          틀린다 — 경보와 교정의 계약은 "한 글자도 다르지 않다" 가 아니라
#                          "같은 입력 집합 안에서 같은 판정" 이다(PR #336 본문 "쌍둥이 정합에
#                          남긴 비대칭" 절). 타 레포 참조는
...
no_sub "(#331) 닫힌 이슈에 사람 게이트가 살아 있으면 조용하다 — 스윕도 무편집" \
  "$tmp/out" "불일치 #94"
no_sub "(#371 의도된 잔여) 닫는 이슈가 어느 목록에도 없으면 조용하다 — 절단 사각은 경보 예산 밖" \
  "$tmp/out" "불일치 #95"
