...
# ── ㉒-b (#244) 계정 전체 탐색은 세 정지 라벨을 전부 훑는다 ───────────────
...
# 부정 라벨은 넣지 않는다(#21) — 긍정 라벨 세 번을 합집합(sort -u)한다.
setup "hold:ladder,agent-ready" 200 0
WORKDIR="$tmp/noscope"
echo '[]' > "$tmp/ladder.json"; echo '[]' > "$tmp/human.json"; echo '[]' > "$tmp/policy.json"
printf 'owner/repo\n' > "$tmp/search"
run
check "탐색: needs-human 쿼리 유지"   "$(grep -q 'search issues label:needs-human is:open is:issue' "$tmp/gh.log" && echo ok || echo no)"
check "탐색: hold:ladder 도 훑는다"    "$(grep -q 'search issues label:hold:ladder is:open is:issue' "$tmp/gh.log" && echo ok || echo no)"
check "탐색: hold:policy 도 훑는다"    "$(grep -q 'search issues label:hold:policy is:open is:issue' "$tmp/gh.log" && echo ok || echo no)"
check "탐색: 부정 라벨은 안 쓴다(#21)" "$(grep -q -e 'search issues .*-label:' -- "$tmp/gh.log" && echo no || echo ok)"
