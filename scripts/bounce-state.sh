...
#         채널 본문(`마커: #N — 사유`)이 전부 여기 들어온다(불가침).
#
...
# **단 표식이 없는 꼬리는 둘째 겹이 못 받는다** — ⒜ 가 구분자 앞 공백을 0~1칸만 보므로
# `재디스패치 : E2E 실패`·`재디스패치  (round 2) 사유`(공백 2칸)·탭·` [round 2]` 는 `ok` 로
# 샌다(main 은 전부 `bounced` — #251 이 연 회귀, 파생 #299 가 닫는다).
# 그 두 겹의 대가는 **⒜ 가 회귀해도 대부분의 칸이 조용히 ⒝/⒞ 로 받아진다**는 것이라,
# 테스트는 ⒜ 를 무력화한 뮤턴트(MUT-A)에서 **⒜ 가 혼자 지키는 칸**(` 3건을 검토했습니다`)
# 하나만 뒤집히고 나머지는 안 뒤집힌다는 사실까지 `want` 열로 못박는다.
#
