...
# 안 의심한다):
#   · **근사인 것**: `완료`·`해소` 어휘로 해소 보고를 가르는 것은 근사다. 그래서 기계
#     채널(⒜ 콜론 형태)은 이 예외의 사정권 밖에 두고, 부정 접두(`미해소`·`불완료`)를
#     빼고, 손으로 쓴 반송이라도 `#N`·`(`·대시·숫자로 시작하면 ⒜ 가 먼저 잡는다 —
#     예외가 닿는 범위는 "마커 + 산문" 한 칸뿐이다.
...
# 두 축의 가드는 각각 `# MUT-1:`(①)·`# MUT-2:`(②) 로 표시돼 있고, 테스트가 그 줄만
# 옛 값으로 되돌린 사본을 만들어 **서로 다른 케이스가 빨개지는지**를 실측한다.
#
       ...
       | select(.value as $x
           ...
           | ($bm | any(. as $m |
               ...
               and (($line[($m | length):]) as $t
                    ...
                    # 해소 보고 표식(#251 ②) — 부정 접두(`미`·`불`)가 붙으면 해소가 아니다.
                    | ($t | test("(^|[^미불])(완료|해소)")) as $resolved
