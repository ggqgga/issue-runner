...
# 사고로 되돌아간다. 낱말 경계는 반대 방향으로만 틀린다: 경계가 모호한 형태(마커 + 공백
# + 평범한 명사)는 `bounced` 로 떨어져 정체될 뿐 안전망이 새지 않는다. 전수 격자는
# `scripts/tests/bounce-state.test.sh` 의 `want` 열에 못박혀 있다(PR#202 교훈).
#
