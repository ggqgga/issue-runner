...
# **GitHub 라벨·PR 상태만 읽는다**(로컬 상태 파일 없음 · 쓰기 0 — 순수 읽기). 세 루프의
# ④ Report 가 이 출력을 그대로 붙이고, 사람도 손으로 친다.
#
...
#
# ★버킷 정의 — 이 주석이 SSOT★ (스킬 문서·README 가 산문 대신 여기를 가리킨다)
#   버킷은 `references/state-machine.md`(#393) 의 상태 행을 대시보드 칸으로 접은 것이다 —
#   상태·소유·회수는 그 표, 여기는 판별식. 이름은 **"누가 들고 있나"** 다(#276): `X대기` =
#   그 루프가 집기 전, 루프 이름 = 그 루프가 지금 들고 있음. 표시 순서는 사다리 앞→뒤.
#
#     줄             라벨                              jq 키
...
#   하나 이하**다(전이 표 SSOT = transition.sh 상단). `needs-human` 과 `hold:*` 는 직교하는
#   일시정지 플래그 — 둘 중 하나라도 붙어 있으면 게이트(#242)가 빼므로 루프가 안 집는다.
#   PR 쪽 라벨은 이슈 칸의 미러다 — **6쌍**(#281·#282, 코드의 `mirror_pairs`): 이슈
#   `agent-ready` 만 ↔ PR `flow:agent-ready` · `agent:claimed` ↔ `flow:claimed` ·
#   `flow:verify`·`verifying`·`flow:ready`·`harvesting` 은 같은 이름. `flow:ci`·`flow:codex` 는
#   PR 에만 있는 워커 내부 단계라 이슈 미러가 없다(= 미러 불일치 판정에 참여하지 않는다).
#   **대기·issue-runner 줄도 연결 PR 을 `← PR #n` 으로 그린다**(#282) — 이슈 칸과 **일치할
#   때만**(불일치는 warn `미러 불일치`, 라벨 0 PR 은 아래 인계 전 창 규칙). 예외 하나 —
#   이슈 대기 칸 + PR `flow:claimed` 는 재claim 대기 모양이라 대기 줄에 붙는다.
#
#   이슈는 OPEN 기준. **한 이슈는 한 버킷** — 위에서 아래로 첫 매칭:
#     1. needs-human — `needs-human` ∪ (`hold:conflict` ∧ `full-cycle`) (#244·#345).
#                  괄호는 `<사다리 위치>, <사유>[, 질문 없음][, PR #n]` — 사유는 `hold:*` 의
#                  접미(`conflict`·`policy`·`ladder`), 여럿이면 정렬해 `, ` 로 잇고 하나도
#                  없으면 `사유 없음` + note `사람이 직접 세운 정지`. `질문 없음`(#157) =
#                  사유가 `policy`·`conflict` 인데 `<!-- hold-note: <그 사유> -->` 마커 코멘트가
#                  없음(마커는 **지금 붙어 있는 사유**로 가린다, #160). `ladder` 는 대상 아님.
#                  `eligible-issues.sh` 의 `blocker_state_of` 가 같은 집합을 읽는다.
#     2. 테스트   — `테스트` 라벨(deploy-cycle ⑤ 가 발행, BoDAT #5197). 사람 결정이 아니라
#                  **밟을 비용**(TEST 워커 런·시간 창·프로덕션 관측)이 남은 것 — 사용자가
#                  `e2e-test` 스킬을 부를 때 비운다. 어느 루프도 안 집는다.
#     3. 배포대기 — `deploy-wait` 라벨, 또는 **사다리 라벨이 0개일 때만** 제목이
#                  `배포 대기`/`배포 검증` 으로 시작(라벨 도입 전 폴백). 판정은
#                  `^배포 (대기|검증)` — 콜론을 앵커로 걸지 않는다(근거 §4).
#     3. 보류     — `hold:*` 가 하나 이상인데 `needs-human` 이 없다(#244·#345).
#                  **단계 라벨보다 앞이다.** 괄호는 사유 병기 — `#4772(ladder)`. **단독**
#                  `conflict` 는 재개 횟수/상한을 잇는다 — `#43(conflict, 0/1)`(횟수 =
#                  `<!-- conflict-resume: N -->` 마커 코멘트 수, 상한 =
#                  `CONFLICT_RESUME_LIMIT`, #346). policy·ladder 가 동존하는 conflict 는 사유만
#                  — `#35(conflict, policy)`(스윕이 재개를 거부한다; 술어
#                  `conflict_resumable_holds`, lib/loop.jq).
#                  조회는 질문 조회와 같은 경로·상한(`HOLD_NOTE_MAX`)·3상태를 타고 상한은 질문
#                  후보부터 채운다. 못 셌으면 횟수를 생략하고 warn `재개 횟수 미확인`.
...
#                  주석) — 여기 jq 는 옮겨 적은 것이다. 한쪽을 고치면 다른 쪽도 같이 고쳐라.
#                  블로커 **상태**는 이미 받은 목록 안에서만 본다(추가 gh 호출 0): 같은 레포
#                  열린 이슈면 `OPEN`(그 이슈의 버킷명이 곧 사유) · 열린 PR 이면 `OPEN PR` ·
#                  둘 다 아니면 해제(닫힘·머지·미존재를 구분하지 않는다). 목록 밖 블로커는
#                  해제로 보인다(fail-open, 근거 §5).
...
#   위 어느 라벨도 없는 열린 이슈는 루프 밖 — 세지 않는다. `열림 N` = 1~10 버킷의 합이지
#   레포의 열린 이슈 총수가 아니다(`막힘` 은 `대기` 에서 옮겨 온 것이라 합은 안 변한다).
#
#   창(`--since`) 안에서만 세는 세 줄 — 버킷이 아니라 교차 집계다(중복 등장 가능):
#     실패     — head 가 `agent/issue-*` 인 PR 이 `closedAt` 창 안 + `mergedAt` null +
#                PR 라벨에 `dup` 없음
#     중복종료 — 같은 조건인데 PR 라벨에 `dup` 이 있는 것. 판정은 **PR 라벨**이지 코멘트
#                마커가 아니다. 실패에서 **빼고** 이 줄로 옮긴다(두 줄에 겹쳐 세지 않는다).
#     파생     — `createdAt` 창 안 + `spinoff` 라벨인 열린 이슈(제목 휴리스틱을 쓰지 않는다)
#
#   승격 대기 — `repos/<repo>/branches/release` 가 있으면 `compare/release...<기본브랜치>` 의
#     `ahead_by`. release 가 없으면 `승격 대기 —`. (closeout ④ Report 는 같은 수를
#     `git rev-list` 로 세지만, 이 스크립트는 로컬 체크아웃에 의존하지 않는다.)
#
# ★warn 정의 — 불변식 위반. **보고만 하고 교정하지 않는다**★
#   warn 은 **루프가 교정 가능한** 불변식 위반만이다(#188·#190 — 근거 §6).
#   · 무소속 PR      — 열린 PR + head 가 `agent/issue-*` + PR 라벨에 `full-cycle` 없음(#246) +
#                      PR 라벨에 단계 라벨(`pr_stage_labels`)이 **하나도 없고** PR 도 연결 이슈도
#                      **정지 라벨**(`needs-human` ∪ `hold:*`)을 안 달았음 → 어느 루프도 안 문다.
#                      head 가 `agent/issue-*` 가 **아니거나** `full-cycle` 이 **붙은** 후보는
#                      warn 이 아니라 note `사람 세션 PR` 로 강등한다(근거는 코드의
#                      `orphan_base`/`$ohuman` 주석 — 여기는 정의, 거기는 근거).
#                      **인계 전 창(#282)**: `flow:claimed` PR 은 창과 무관하게 issue-runner 줄에
#                      붙고, 아래 창 규칙은 라벨 0 PR 에만 남는다. 그 후보 중 연결 이슈가
#                      **issue-runner 버킷**이고(라벨이 아니라 버킷으로 본다) PR `createdAt` 이
#                      `HANDOFF_GRACE_MIN`(기본 90) 분 미만이면 warn 이 아니라 **issue-runner 줄**에
#                      `← PR #n(인계 전)` 으로 붙는다 — 후보 집합을 **분할**하므로 표시와 warn 은
#                      항상 배타다. 창을 넘기면 무소속 warn 에
#                      `(agent:claimed 인데 <N>분 경과 — 워커 사망 의심)` 이 덧붙는다.
#                      그 `<N>분` 은 **가장 최근 `agent:claimed`
#                      labeled 이벤트**(이슈 타임라인) 기준이고(#177) 창 분할만 PR 나이로 한다.
#                      시각을 못 얻으면 **warn 은 유지**하되 두 문구를 가른다(#181): 조회 실패·
#                      이벤트 없음 → `(agent:claimed 인데 경과 미상 — 확인 필요)` ·
#                      `CLAIM_TIME_MAX` 초과로 조회 안 함 → `(agent:claimed 인데 경과 미상 — 조회 상한)`.
#   · 워커 사망 의심 (#282)
#                    — issue-runner 줄에 `flow:claimed` PR 이 붙은 이슈인데 그 PR 이 인계 전 창
#                      밖이고 **claim 경과**(위와 같은 기준)도 창 이상. 후보는 PR 나이로 고르고
#                      판정은 claim 경과로 한다. 시각 미상 처리는 위와 같다. 문구:
#                      `워커 사망 의심 #i(rs) ← PR #n(rs) — agent:claimed 인데 N분 경과`.
#   · 질문 유무 미확인 — **needs-human 버킷**의 `hold:policy|conflict` 이슈인데 질문(hold-note)
#                      코멘트의 유무를 못 봤다(조회 실패·파싱 실패·코멘트 100건 상한·
#                      `HOLD_NOTE_MAX` 초과). "질문 없음" 으로 접지 않는다.
#   · 재개 횟수 미확인 (#346) — **보류 버킷**의 `hold:conflict` 이슈인데 재개 횟수를 못 셌다
#                      (같은 네 사유). 보류 줄엔 횟수 없이 `#N(conflict)`.
#   · 블로커 needs-human — `막힘` 버킷의 블로커가 **needs-human 버킷**이면 한 줄(#248; 문구는
#                      버킷 표시 이름(#276)을 그대로 쓴다 — `블로커 needs-human #N`).
#                      `배포대기` 블로커도 같은 규칙으로 `블로커 배포대기 …`.
#                      묶음 단위는 **블로커**다(하위는 번호 내림차순). 나머지 버킷 블로커와 PR
#                      블로커는 warn 이 아니다 — 루프가 처리 중이다.
#   · 단계 라벨 중복 — 이슈에 사다리 라벨 2개 이상.
#   · 미러 불일치    — 이슈 칸과 **열린** 연결 PR 의 미러 라벨이 6쌍으로 어긋남 — 양방향.
#                      연결 PR 이 없으면 warn 아님. 워커 칸 두 쌍은 **PR 이 그 라벨을 달고 있고
#                      이슈가 정지 중이 아닐 때만** 대조에 든다(근거는 코드의 `mirror_scoped`).
#                      이슈 대기 칸 + PR `flow:claimed` 는 재claim 대기라 warn 이 아니다.
#                      문구는 양쪽 칸을 그대로 — 이슈는 이슈 라벨 이름(사다리 라벨 0 이면
#                      `agent-ready 만`, 그것도 없으면 `단계 없음`), PR 은 PR 라벨 이름.
#   · 정지 미러 불일치 (#265)
#                    — 이슈엔 정지 라벨(`needs-human` ∪ `hold:` **접두** — SSOT 는 BUILD_JQ 의
#                      `stops_of`)이 **하나도 없는데** 짝이 되는 **열린** PR 에 남아 있음.
#                      **해제 방향만** 본다. 짝은 head `agent/issue-N` 의 그 `N` 이
#                      `closingIssuesReferences` 안에 있을 때뿐이고, 그 PR 이 닫는 이슈가 **전부**
#                      깨끗할 때만 낸다(전건 판정은 열린 이슈 ∪ 닫힌 이슈, #331 — gh 호출 0).
#                      맨몸 `needs-human` 만 남은 PR 은 대상 밖. 짝이 되는 열린 PR 을 **전부**
#                      대조한다. 교정: `resume-sweep.sh` 의 정지 미러 정리 갈래(근거 §8).
...
#                      `닫힌 이슈`(최근 200건) 목록은 후보에서 **뺐다**(#292 — 근거 §9).
#   · 에픽 닫힌 leaf 조회 실패 (#292) — 에픽 절의 검색 스코프 조회가 실패했다(쿼터·네트워크·
#                      응답 형식·**조용한 빈 결과**). 그 레포의 에픽 줄은 `종료 미상` 이 된다.
#                      탐지 범위의 사각(⑵ 조용한 빈 결과)은 근거 §10.
#   · 연결 이슈 종료 — 열린 PR 인데 연결 이슈가 CLOSED. `Refs` 부분착지면 정상 — 사실만 한 줄.
#                      (OPEN 여부는 이미 받은 열린 이슈 목록의 멤버십으로 본다 — 목록 상한 200
#                       밖의 열린 이슈는 CLOSED 로 오인될 수 있다.)
#
# ★note 정의 — 불변식 위반이 **아닌** 사실. 루프가 집을 수 없으니 warn 이 아니다★
#   · 사람이 직접 세운 정지 — **needs-human 버킷** 이슈에 `hold:*` 라벨이 하나도 없음(#244).
#                      판별은 버킷 기준이다(needs-human 이 가장 앞이라 배포대기·테스트 제목이어도
#                      그 라벨이면 여기로 온다).
#   · 사람 세션 PR   — 무소속 PR 의 나머지 조건은 다 맞는데 head 가 `agent/issue-*` 가 아니거나
#                      PR 라벨에 `full-cycle` 이 붙은 열린 PR(사람 세션이 판 `feat/*` 등 — #246).
#                      warn 에서 빼되 존재는 남긴다 —
#                      `note N` 줄 아래 한 줄씩. 괄호 안 사유는 걸린 축을 적는다(`full-cycle 라벨` ·
#                      `agent/issue-* 아님` · 둘 다면 ` · ` 로 병기).
#
# ★에픽 절★ (#260) 열린 `epic` 라벨 이슈마다 leaf(하위) 진척을 한 줄로. `승격 대기` 줄 바로 위,
#   `파생` 줄 아래. leaf 판정 — 열린·닫힌 이슈 본문의 **줄 시작**의 `epic\s+#N`(대소문자 무시,
#   줄 앞 공백 허용, 첫 매치). 산문 속 `… epic #N …` 은 leaf 로 잡히지 않는다.
#   한 줄 형식(leaf ≥ 1): `#<에픽> <종료>/<전체> · <버킷 분포> · <P 분포>`. 두 분포 다 **열린**
#   leaf 만 센다(P 라벨 없는 leaf 는 안 센다). 버킷 분포는 10버킷을 7칸으로 접는다 —
#   `agent:claimed`·`flow:verify`·`verifying`·`flow:ready`·`harvesting` 은 `진행`, 나머지
#   (`막힘`·`대기`·`반송대기`·`보류`·`needs-human`·`배포대기`)는 그대로. 0건인 칸은 생략.
#   leaf 0 인 에픽은 `#<에픽> leaf 없음(Epic 줄 미부착)` 한 줄. **닫힌 leaf 조회가 실패했으면**
#   `#<에픽> 종료 미상(닫힌 leaf 조회 실패)` 한 줄이다(#292) — 버킷·P 분포는 그대로 붙고,
#   `에픽 leaf 전부 종료` warn 도 **그대로 뜬다**(근거 §10).
#   warn 2종: · 에픽 leaf 전부 종료 — leaf ≥1 전부 닫힘인데 에픽이 열려 있다. 꼬리 셋(#441):
#       스윕 마커(`<!-- epic-sweep -->`) 없으면 `닫아라(에픽 스윕 대상)` ·
#       있으면 `사람 몫(스윕 되돌림)` · 못 봤으면 `스윕 대상 여부 미확인(<사유>)`
#       (상한 `EPIC_MARK_MAX`).
#     · 에픽 내 P 혼재 — 열린 leaf 의 P 라벨이 둘 이상 갈린다.
#   파생 병기 — `파생` 줄 항목의 `(Epic #N)`/`(에픽 없음)` 은 **그 레포 이슈 본문 어딘가에 실제
#   `Epic #N` 줄이 하나라도 있을 때만** 붙인다(열린·닫힌 leaf 모두 본다).
#   ★닫힌 leaf 의 창★ (#292)
#   `gh issue list --state closed --search '"Epic #" in:body' --limit $EPIC_CLOSED_LIMIT`
#   (레포당 1회 · **열린 에픽이 1건 이상일 때만**). 그 결과를 종전의
#   `--state closed --limit 200`(core API) 목록과 **합집합**으로 쓴다 — 검색은 후보를 좁히고
#   판정(`epic_of` 한 벌)은 안 한다.
#
...
#   degrade. 질문 코멘트 조회(#157) 실패는 그 이슈만 `질문 없음` 표시를 **생략**하고 warn
#   `질문 유무 미확인`. 에픽 닫힌 leaf 조회(#292) 실패는 그 레포의 에픽 줄만 `종료 미상` 으로
#   degrade 하고 warn `에픽 닫힌 leaf 조회 실패` — 판정은 ⑴ 종료코드 ⑵ 응답이 배열인가
#   ⑶ core API 목록과의 교차확인(조용한 빈 결과) 셋을 전부 본다.
# ★환경 실패 처리★ 레포와 무관한 실패(창 시각 계산 불가·jq 부재·집계/렌더/직렬화 jq 실패)는
#   **stdout 에도** `파이프라인 — 스냅샷 실패: <사유>` 한 줄을 남기고 exit 1 한다.
#
# ★환경 변수★ (형식이 틀리면 전부 **환경 실패**로 죽는다 — jq 에 넘겨 레포별 "집계 실패" 로
#   위장되지 않게)
#   `HANDOFF_GRACE_MIN`     인계 전 창(분, 기본 90, 0 이상 정수). `0` 허용.
#   `HOLD_NOTE_MAX`         레포당 질문(hold-note) 코멘트 조회 상한(기본 50, 0 이상, #157).
#                           보류 conflict 의 재개 횟수 조회(#346)도 같은 상한을 나눠 쓴다.
#   `CONFLICT_RESUME_LIMIT` 보류 줄 `#N(conflict, n/상한)` 의 분모(#346). 값은
#                           `scripts/lib/constants.sh` 한 자리(#427) — 재개 스윕과 같은 상수.
#   `CLAIM_TIME_MAX`        레포당 `agent:claimed` 시각(타임라인) 조회 상한(기본 20, 0 이상,
#                           #177). **고유 이슈 수**를 센다(#181).
#   `EPIC_CLOSED_LIMIT`     에픽 닫힌 leaf 조회의 행 상한(기본 1000, **1 이상** 정수, #292).
#                           상한에 닿으면 warn `목록 상한 <N> 도달 — 창 절단 가능(에픽 닫힌 leaf)`.
#                           다른 상한과 달리 `0` 을 금지한다(근거 §10).
#   `EPIC_MARK_MAX`         레포당 스윕 마커 코멘트 조회 상한(기본 20, 0 이상, #441). 넘는
#                           후보는 조회하지 않고 `스윕 대상 여부 미확인(조회 상한)` 으로 남는다.
#
# ★gh 호출 예산★ 레포당 열린 이슈 1 + 닫힌 이슈 1 + 에픽 닫힌 leaf 1(**열린 에픽이 있을 때만**)
#   + PR 목록(open/closed) 2 + release 1 + 기본 브랜치 1 + compare 1 = 최대 8(에픽 없는 레포 7).
#   블로커 판정(#248)·에픽 leaf 판정(#260)은 이 예산을 **한 호출도 늘리지 않는다**.
#   **예외 넷**(이슈·PR **개별** `gh view` 는 여전히 금지 — N+1):
#     ①(#157) needs-human 버킷의 `policy`·`conflict` 이슈마다 `gh issue view --json comments` 1건
#     ②(#177·#282) 사망 의심 꼬리표 후보와 `워커 사망 의심` 후보마다 이슈 타임라인
#              (`gh api .../timeline --paginate`) 1건
#              (창 안 PR 은 조회하지 않는다, 상한 `CLAIM_TIME_MAX`)
#     ③(#292) 에픽 닫힌 leaf 조회 — **유일한 검색 사용처**
#     ④(#441) `에픽 leaf 전부 종료` warn 후보마다 코멘트 전량 조회 1건(상한 `EPIC_MARK_MAX`)
#   `gh search` / `gh issue list --search` 는 **원칙 금지**(인덱스 지연 + 부정 라벨 오파싱, #21) —
#   예외 ③ 이 성립하는 세 조건은 근거 §11. 라벨 필터는 전부 jq 로 한다. 목록은 `--limit 200`
#   상한 — `--since` 를 크게 잡으면 닫힌 PR 이 잘려 "실패" 가 조용히 누락될 수 있다.
#
# 근거: references/scripts-rationale.md loop-status.sh §1–§11
set -uo pipefail
...
# 묶는다. 10버킷을 7칸으로 접는다: claimed/verify/verifying/ready/harvesting → `progress`(사람용 `진행`),
