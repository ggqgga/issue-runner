...
| def pr_of($n): ($po | map(select(.issue == $n)) | if length > 0 then .[0] else null end);
...

  {
    ...
    warns: (
      ...
      # 두 번째였다).
      #
      # ★짝짓기 규칙 — `linked()` 보다 **좁다**(교정 갈래 `resume-sweep.sh` 의 ④ 와 **같은
      # 입력 집합 안에서 같은 판정**이어야 한다. 경보와 교정의 대상 집합이 갈리면 한쪽이
      # 거짓말을 한다. "한 글자도 다르지 않다" 가 아니라 "같은 입력 안에서" 인 이유는 아래
      # ⑶ 끝의 의도된 잔여 — 두 갈래의 **입력 집합 자체**가 다르다: 여기는 이미 받은 목록,
      # 저쪽은 번호로 직접 조회)★
      ...
      #      **의도된 잔여(#371 ⓑ — 계약 문언을 좁혔다):** 남는 비대칭은 **둘 중 어느 목록에도
      #      없는 번호**(각 200건 상한 절단 — 최근 200건 밖의 오래 닫힌 비에픽 이슈)뿐이다.
      #      그때는 "못 봤다" 이므로 조용한 쪽으로 틀린다 — 교정 갈래는 번호로 직접 조회하므로
      #      절단이 없고, 그 칸에서 경보는 침묵하는데 교정은 돈다. 이것을 **경보 쪽에서 닫지
      #      않는** 근거 둘: ⓐ 이 블록의 선재 계약은 "이미 받은 목록 안에서만 판정 · 추가 gh
      #      호출 0"(#190·#260 예산)이고, 그 번호를 알려면 PR 마다 `gh issue view` 가 필요하다.
      #      ⓑ warn 은 루프가 교정 가능한 위반일 때만 낸다(#190) — 이 칸의 오차 방향은
      #      "올바른 교정이 경보 없이 돈다" 라 사람이 조치할 것이 없고, 못 본 것을 추측해서
      #      울리면 조치 불가능한 잡음이다. 그래서 두 술어의 계약은 "한 글자도 다르지 않다"
      #      가 아니라 "**같은 입력 집합 안에서** 같은 판정" 이다(PR #336 본문 "쌍둥이 정합에
      #      남긴 비대칭" 절이 이 칸을 처음 못 박았고, 테스트 `#95`/`closes=[95, 9999]` 가
      #      의도된 잔여로 문다). 타 레포 참조(`Closes owner/other#N`)는 *부재*가 아니라 **충돌**로
