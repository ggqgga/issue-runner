...
#
# **이슈 코멘트도 같은 자리에서 읽는다**(#397). REST 는 PR 코멘트를 `issues/{n}/comments` 로
# 주므로(PR 은 이슈의 부분집합) 이슈 번호를 그대로 넘기면 된다 — `resume-sweep.sh` 의 마커
# 카운트(`ladder-resume`·`mirror-retry`)와 재심 마커 판정이 그 소비자다. 그쪽이 쓰던
# `gh issue view --json comments` 는 **첫 100건 상한**이라, 코멘트가 100건을 넘는 이슈에서는
# 마커가 영영 0으로 보여 상한이 안 걸리고 같은 코멘트만 계속 쌓였다(이 파일이 없애려던 함정의
# 이슈 판). 이름의 `pr` 은 첫 소비자에서 온 것이고 계약은 "이슈/PR 번호" 다.
#
