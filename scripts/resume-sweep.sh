...
#   · 펜스는 CommonMark 대로 **줄 머리**(들여쓰기 ≤3칸)에서만 연다/닫는다. 여는 펜스의
#     **문자(백틱 또는 물결)와 길이**를 이름있는 그룹(`(?<f>```+|~~~+)`)으로 기억해 두고,
#     닫는 줄은 같은 문자로 **그 길이 이상**(`\k<f>`)이며 뒤에 **공백만** 올 때만(`[ \t]*`,
#     그 뒤 줄끝/문자열끝) 닫힌 것으로 본다 — 다른 텍스트가 붙은 줄(`` ``` not-a-close ``)
#     이나 더 짧은 런(사중 펜스 안의 삼중 줄)은 닫지 못하고 지나친다. 닫는 펜스가 없으면
#     **문서 끝까지**가 코드다(`$` 대안 — 덜 지우는 쪽이 아니라 CommonMark 자체가 그렇다).
#     여는 펜스 줄의 뒤(info string)는 그대로 허용한다. jq(Oniguruma)의 `^` 는 줄 머리가
#     아니라 **문자열 머리**라 `(^|\n)` 로 직접 쓴다(실측 — `test("^```")` 는 2행의 펜스에
#     거짓이다).
...
#   · 알려진 느슨함(그대로 남긴다 — 숨기지 않는다): 닫는 펜스 패턴 뒤에 `` `*~* `` 를 붙여
#     둬서, 형식상으로는 백틱 펜스를 물결로(또는 그 반대로) 닫는 것도 허용된다. CommonMark
#     는 여는 것과 같은 문자만 인정하지만, 그런 혼용은 실제 코멘트에 나타나지 않고 이 좁은
#     허용이 마커를 잘못 세는 방향(과소/과다)으로 틀리지도 않는다(양쪽 다 여전히 "코드"로
#     처리된다) — 정확성보다 실용을 택한 선택이다.
