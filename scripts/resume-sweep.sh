...
# 인용(코드펜스·인라인 코드) 안의 마커를 제어 신호로 세지 않기 위한 jq 정의는
# `jq-unquote.sh` **한 자리**로 옮겼다(#301) — `reissue-pr.sh` 의 `회차 허용:` 문형
# 판정이 같은 계산기를 써야 하고, 복사하면 두 판정이 조용히 갈린다. 규칙·실측 근거(#197·
# #230 의 격자)는 그 파일 머리말에 그대로 있다.
# 부재·빈 출력은 **fail-loud** 로 끊는다 — 정의 없이 돌리면 `unquoted` 미정의로 모든
# jq 판정이 파싱 실패가 되고, 그건 "인용을 안 걷어낸" 게 아니라 판정 전체가 조용히 꺼지는
# 것이다(실행비트 누락 시 exit 126 로 빈 값이 되는 경로 — #173 교훈).
JQ_UNQUOTE=$("$SCRIPT_DIR/jq-unquote.sh" 2>/dev/null) || JQ_UNQUOTE=""
if [ -z "$JQ_UNQUOTE" ]; then
  echo "resume-sweep: jq-unquote.sh 에서 인용 제거 정의를 못 얻었다 — 경로·실행비트 확인" >&2
  exit 1
fi
