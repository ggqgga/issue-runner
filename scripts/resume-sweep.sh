...
# 사라져(재개 횟수 과소집계 → 상한이 안 걸리는 무한 재개) 원래 버그보다 나쁘다:
#   · 펜스는 CommonMark 대로 **줄 머리**(들여쓰기 ≤3칸)에서만 연다/닫는다. 산문이 한 줄
#     안에서 백틱 세 개를 언급하는 일(이 주석이 바로 그렇다)이 흔한데, 줄 앵커가 없으면
#     그런 언급 두 개 사이의 맨몸 마커가 통째로 지워진다. jq(Oniguruma)의 `^` 는 줄 머리가
#     아니라 **문자열 머리**라 `(^|\n)` 로 직접 쓴다(실측 — `test("^```")` 는 2행의 펜스에
#     거짓이다). 닫히지 않은 펜스는 짝이 없어 아무것도 안 지운다(덜 지우는 쪽으로 실패).
#   · 인라인은 CommonMark 의 코드 스팬 규칙대로 **구분자 길이를 맞춘다** — 여는 백틱 런
#     `(`+)` 과 **같은 길이**의 닫는 런 `\1` 까지가 한 스팬이고, `(?!`)` 로 그 이상은 안
#     먹는다. 백틱을 하나씩 짝지으면(첫 회차가 그랬다) 짝수 길이 구분자가 "빈 스팬 두 개"
#     로 갈려 **구분자만 지우고 알맹이(마커)를 맨몸으로 남긴다** — 인용인데 신호로 세진다
#     (실측 — #197 마감 검증: 단일·삼중·펜스는 걸러졌는데 이중 백틱만 셈=true).
