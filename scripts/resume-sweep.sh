...
if [ -f "$scope_file" ]; then
  ...
  #      ⓐ 가 못 보는 ④ 의 표적을 이 축이 덮는다. 기계가 만드는 정지 미러는 **언제나**
  #      `needs-human` 을 동반하므로(정지 세 전이는 `--reason` 필수 → `hold:<사유>` 와 쌍,
  #      `sweep_hold_mirror` 의 ⑵-b 주석) 오늘의 기계 산출은 전건 이 축에 잡힌다.
  #      놓치는 것: `needs-human` **없이** `hold:*` 만 달린 PR(격자 ⑦·⑩ 의 모양) 가운데
  #      아래 루프의 세 라벨(`needs-human`·`hold:ladder`·`hold:policy`) 어느 것도 아닌
  #      사유(`hold:conflict`·`hold:manual` 따위)만 남은 것.
  #      **⑵-b 가 이 모양을 막아 주지 않는다** — 그 관문은 `hold:` 접두가 *하나도 없을* 때만
  #      거르므로 `hold:conflict` 하나만 남은 PR 은 통과해 **실제로 뗀다**(격자 ⑦ `hold:ladder`·
  #      ⑩ `hold:manual` 이 그것을 단언한다). 그러니 이 사각은 "어차피 안 뗀다" 가 아니라
  #      **탐색이 못 찾아서 교정이 안 도는** 칸이다: 오늘 기계는 이 모양을 못 만들고
  #      (`transition.sh` 의 정지 세 전이는 `pr_add="needs-human hold:$reason"` 로 **쌍**을,
  #      해제 전이는 `pr_rm="… needs-human $HOLD_ALL"` 로 **함께** 뗀다) 사람만 만들 수 있는데,
  #      그 레포에 세 정지 라벨이 이슈에도 PR 에도 0건이면 경보(`loop-status` 의 `정지 미러
  #      불일치`)는 `--repo` 로 찔렀을 때 울리는데 계정 전체 스윕은 방문조차 안 한다 —
  #      **의도된 잔여 비대칭**이고, 오늘 그 방향으로 틀리는 것은 사람이 세운 브레이크를
  #      보존하는 쪽이라 해롭지 않다.
  #      넓히지 않는 이유: `hold:` 는 **접두**라(사유가 열려 있다 — #242) `--label` 로 질의할
  #      수단이 없고, 열거로 흉내 내면 이 파일이 버린 그 실수를 되살린다. #244 가
  #      `needs-human` 을 기계 정지에서 뺀 뒤로는 아래 루프가 **세 라벨 각각**을 두 축으로
  #      돌리므로(`hold:ladder`·`hold:policy` 는 PR 축에서도 질의된다) 오늘 기계가 만드는
  #      정지 라벨은 어느 쪽이든 잡힌다 — 라벨 목록이 늘면(또는 접두 질의 수단이 생기면)
  #      아래 `for _lbl` 한 곳만 늘린다(그 전까지 이 주석이 겹의 위치를 가리킨다).
  #      참고: 레포가 어느 축으로든 목록에 들면 ④ 는 그 레포의 **열린 PR 전부**를 훑으므로
  #      (`fetch_open_prs` 는 라벨로 안 좁힌다) 남는 사각은 "그 레포에 세 정지 라벨 중
  ...
  # `gh search` 가 1회에서 6회(세 라벨 × 두 축)로 늘었으니 그 노출면도 그만큼 넓다. 다만 그때
  ...
  #
  # 라벨마다 **두 축**(이슈 ⓐ · PR ⓑ, #331)을 돌린다 — 세 라벨 × 두 축 = 6 쿼리. 위 주석의
  # "#244 가 뗀 날 이 축도 함께 옮긴다" 는 바로 이 루프가 답이다: 라벨 목록이 늘면 PR 축도
  # 같은 라벨로 따라가므로 겹이 어긋나지 않는다.
