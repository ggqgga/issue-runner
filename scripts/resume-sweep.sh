...
#   다르다(입력 집합 — #371 ⓑ 의도된 잔여): ⑶ 전건 게이트의 **입력**이 갈린다. 여기는
#     `read_labels_state` 로 닫는 이슈를 번호로 **직접 조회**해 절단이 없고, 경보는 이미 받은
#     목록(열린 이슈 ∪ 닫힌 이슈 `--limit 200` ∪ 에픽 leaf 검색) 안에서만 판정한다. 그래서
#     최근 200건 밖의 오래 닫힌 비에픽 이슈를 함께 닫는 PR 은 **경보는 침묵하고 교정만
#     돈다**. 경보 쪽에서 닫지 않는 이유는 `loop-status.sh` ⑶ 주석에 있다(예산 계약 #190·#260
#     + warn 은 교정 가능한 위반만 #190 — 오차 방향이 "올바른 교정이 경보 없이 돈다" 라
#     무해). 처음 못 박은 자리는 PR #336 본문 "쌍둥이 정합에 남긴 비대칭" 절.
#
# ★짝짓기 규칙 — `loop-status.sh` 의 정지 미러 warn 과 **같은 입력 집합 안에서 같은 판정**★
# (경보와 교정의 대상 집합이 갈리면 한쪽이 거짓말을 한다 — 단 입력 집합이 다른 칸은 위
# "다르다(입력 집합)" 의 의도된 잔여다). 짝으로 인정하는 조건 둘:
