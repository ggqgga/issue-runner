...
- `MAX_TIMEBOX_GRACE` — 누적 유예 횟수를 **상태 파일이 아니라** 이슈 코멘트 마커
  ...
  유예 횟수를 물려받지 않기 위해서다. 값의 단위는 횟수지만 **뜻은 시간(유예 창 ≈ 45분)**이다 —
  틱 간격 15분 시절 3회로 잡은 것이 2026-09-13 크론 5분 주기 전환 뒤 15분(설계의 1/3)으로
  줄었고, 수용 기준을 다 마친 워커가 `grace_exhausted` 로 끊긴 실측(bodat #4977: 재디스패치가
  코드 변경 0 으로 PR #5207 만 열며 ~135k 토큰)이 나와 9회로 올렸다(PR #436 → #526). 틱 간격을
  다시 바꾸면 이 값도 같은 비율로 맞춘다.
