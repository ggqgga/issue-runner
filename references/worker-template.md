...
9. 최종 push 후 로컬 CI 를 실행하라:
   ...
   **CI 큐 대기 규율 (#185).** `run-local-ci.sh` 는 즉시 반환하지만 실행 자체는 박스
   전역 **직렬 큐**를 거친다 — 다른 워커의 CI 가 앞서 있으면 몇 분씩 밀리고, 그 대기를
   잘못 다루면 워커가 조용히 턴을 끝내 디스패처에게 **사망으로 오독**된다(실측 사고).
   - **`bin/ci` 를 직접 부르지 마라** — 위 `run-local-ci.sh` 만 써라. 직접 호출은 즉시
     반환하지 않아 툴콜 타임아웃에 걸린다.
   - 결과 확인은 **툴콜마다 한 번씩** `<sha>.result` 파일을 본다. `sleep` 로 붙잡지 마라.
   - **CI 를 겹쳐 걸지 마라** — 걸기 전 `ps -Ao pid,etime,command | grep '[b]in/ci'` 로
     이미 도는 게 없는지 확인하라(같은 worktree 에 두 개가 겹치면 테스트 DB·픽스처를
     공유해 양쪽 다 죽는다).
   - 대기 중에는 CI 와 무관한 일(PR 본문 초안·9-b 사전 리뷰 준비)을 먼저 해 둬라.
   ...
   ```
   - **대기열 / 결과(pass·fail)** — 정상 진행. `.result` 파일이 생길 때까지 기다려라.
   - **폐기 — HEAD 불일치.** 큐는 실행 시점 HEAD 가 건 시점의 SHA 와 다르면 그 SHA 를
     버린다. **기다릴 SHA 는 항상 "지금 HEAD"여야 한다** — 새 커밋을 push 했으면 새
     SHA 로 다시 걸고 새 SHA 의 결과를 봐라. 존재하지 않을 옛 SHA 의 결과를 기다리며
     턴을 붙들지 마라.
   - **중단(INT/TERM) — 짧은 타임아웃이 대기 중인 프로세스를 함께 죽인다.**
     `run-local-ci.sh` 자체는 즉시 반환하지만, 뒤이은 Bash 호출에 짧은 타임아웃을
     걸면 큐 안에서 대기 중인 프로세스까지 함께 끊긴다. **짧은 bash 타임아웃을 걸지
     마라.**

   **턴을 끝내야 하면 조용히 끝내지 마라.** 큐 대기가 길어져 이번 턴에서 결과를 못
   보고 마쳐야 한다면, 마지막 메시지를 반드시
