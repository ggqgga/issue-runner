...
9. 최종 push 후 로컬 CI 를 실행하라:
   ...
   **직렬 큐**에 SHA 를 올리고, 대기+실행이 끝나야 돌아온다. 다른 워커의 CI 가 앞서
   ...
   - **"툴 타임아웃 ≠ 작업 중단"** (#185 재심, 2026-09-11 `bodat#5046` 실측). Bash 툴의
     10분 상한에 걸려 이 호출이 끊겨도 `run-local-ci.sh`/`bin/ci` 프로세스 자체는
     보통 계속 돈다 — 끊기는 것은 **기다리던 툴 호출**뿐이다. 그러므로 툴 타임아웃을
     만났다고 곧바로 "실행이 안 됐다"고 단정해 재큐하지 마라. 아래 순서로 먼저
     **살아 있는 실행**을 찾아라:
     1. `TaskList`/`TaskOutput(block: true, timeout: 600000)` 로 백그라운드 전환
        여부를 먼저 확인하라. 그게 비어 있거나(서브에이전트라 접근 불가) 아무것도
        없으면 `PID=$(ps -Ao pid,command | grep '[b]in/ci' | awk '{print $1}' | head -1)`
        로 `bin/ci` 프로세스를 직접 찾아라 — 이건 "돌기 전에 남이 도는지 보는
        사전 게이트"가 아니라 **이미 띄운 내 실행을 회수할 때만** 쓰는 것이다(아래
        "겹침은 네가 막지 마라" 불릿과 혼동 금지).
     2. **PID 가 살아 있으면(`kill -0 $PID` 성공) 재큐하지 말고 회수하라** — 그
        툴콜 안에서 `while kill -0 $PID 2>/dev/null; do sleep 10; done` 로 폴링하고,
        10분 안에 안 끝나면 다음 툴콜에서 같은 폴링을 다시 걸어라(각 호출이 600초
        이내면 몇 번을 재진입해도 결국 회수된다). 끝나면 `<sha>.result` 파일(경로는
        `run-local-ci.sh` 자신의 출력에 찍힌다 — `~/.claude/.local-ci/<레포 slug>/<sha>.result`)
        을 읽어 판정하라.
     3. **PID 가 이미 죽었고 `<sha>.result` 파일도 없을 때만** 실행이 없었던 것이다 —
        그때만 아래 "미실행" 판정(폐기·중단·유령 티켓 회수)을 따라 같은 SHA 로
        1회 재큐하라. (같은 SHA 를 동시에 처리 중이던 다른 세션이 먼저 끝냈을
        수도 있으니 재큐 전에 결과 파일부터 가볍게 다시 확인하라.)
