...
| S4 | `flow:ready` / `flow:ready` / `✅`(head 이후·`코멘트 스냅샷 N`) | closeout | `verify-pass` | `closeout-eligible.sh`: `✅` 마지막 인덱스 + `finish-classify.sh`=done_verdict(✅ 가 head 커밋 이후임을 증명) + `bounce-state.sh`=ok + 스냅샷 경계 이후 무마커 코멘트 0 + `closeout-ci-pass.sh` + MERGEABLE + ¬harvesting·¬verifying·¬flow:verify·¬needs-human·¬hold:* | `closeout-pick` → S5 | closeout ①-b 스윕(아래 계급 표) · ② Pick 직전 ①-c 보류 해제 방향 판정(`hold-resolve.sh`, #334 — 사람이 판 보류의 결론이 시정이면 반송) |
...
exit 코드별 뜻: **1 = readback 불일치**(편집은 갔는데 되읽은 라벨이 안 맞는다) · **2 = gh 실패**(조회·편집이 아예
안 됐다) · **64 = usage**(인자 형태가 틀렸다 — 데이터가 아니라 **프롬프트·문서 버그**다. 인자를 고치기 전엔 몇
번을 다시 불러도 같은 값이 나오니 재시도가 무의미하다). 셋 다 조치는 위와 같다 — 종료 상태를 바꾸지 말고
`BLOCKED:` 한 줄로 올린다.

**재시도는 다음 틱이다** — 같은 틱 안에서 같은 전이를 다시 돌리지 마라(무한 재시도 금지). 전이는 멱등이라, 반쯤
이동한 상태에 같은 전이를 다시 걸면 이미 맞은 쪽은 no-op 이고 어긋난 쪽만 맞는다 — **재호출이 곧 복구**다. 그래서
다음 틱이 같은 이벤트를 다시 내는 것으로 충분하고, 그 자리에서 손으로 라벨을 옮길 이유가 없다.
