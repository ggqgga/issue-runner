...
- **회수** = 소유 루프가 죽었거나 전이가 반쯤 실패했을 때 그 상태를 다시 정상 궤도로 올리는 주체. `—` 는 회수가
  필요 없는 상태(사람이 푼다 또는 종료). **미정** 은 소유 주체가 없는 칸이고 leaf 이슈 번호를 단다.
...
| 형상 | 왜 아무도 안 집나 | leaf |
|---|---|---|
| `verify-redispatch` exit 1·2 — PR 은 `flow:verify`/`verifying` 상실, 이슈는 `agent:claimed` 유지 | verify-eligible(라벨 없음)·closeout-eligible(✅ 없음/bounced)·eligible-issues(claimed) 전부 제외. SKILL 은 "다음 틱이 잡게" 라 적었으나 주체 없음 | #394 — reconcile 이 전이를 멱등 재실행 |
| 연결 이슈 없는 PR 의 `hold:policy` | resume-sweep 이 이슈만 스캔 → 재심 이벤트가 안 나온다 | #395 — PR 단독 홀드도 재심 대상 |
| `머지 판정` 코멘트가 0건인 초록 PR(handoff 전 사망) | issue-runner ② 는 "CI green·리뷰 없음" 무접촉, ①-b 는 🔄/✅ 전제, finish-classify → active | #396 — `no_verdict` 계급 → 재디스패치 |
| resume-sweep 미러 정리 "양성 증거 못 얻음" warn | 떼지 않고 warn 만 — 재시도 주체 없음 | #397 — 상한 있는 재시도 + 상한 뒤 `runner-held` |
...
다음 틱이 잡게 하는 것이 목적이다(조용히 넘어가지 않는다). 그 "다음 틱" 의 주체는 위 표의 회수 열이다 — 회수 열이
**미정** 인 전이는 그 leaf 가 닫히기 전까지 사람이 눈으로 찾아야 한다.
