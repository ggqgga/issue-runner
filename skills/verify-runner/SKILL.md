...
- `VERIFY_ATTEMPTS_LIMIT = 3` — 같은 PR 검증이 N회 실패(재디스패치)하면 그 다음엔
  ...
  이슈를 낸다. **사람에게 묻지 않는다**). 상한 도달은 `hold:policy` 정지(④ **held**,
  `verify-held --reason policy`)의 사유가 **아니다** — 그 정지는 연결 이슈 부재처럼
  스펙·정책 선택이 남은 경우만이고, 사람 호출(`needs-human`)은 그 정지의 재심이 "사람 몫
...
이 기준이 막는 것 — 새 단언·새 가드·커버리지 추가·상수값·실행 분기.
...
후보당 조회 2회 — 큐가 보통 0~3개라 틱 비용은 무시할 만하다). 사람이 원 이슈에
`회차 허용: +1 — 범위: <한 줄>` 문형이 든 코멘트(줄 머리, 또는 `사람 결정: **ⓐ — ` 같은
머리말 뒤 — 2026-09-12 #283·#244 에 사람이 실제로 적은 형태)를 남겼으면 그 헬퍼가 PR 본문 `verify-attempt` 를
`VERIFY_ATTEMPTS_LIMIT - 1` 로 되돌리고 **그 갱신이 성공한 뒤에만** 원 이슈에 `<!-- round-granted -->`
마커 코멘트를 남긴다(마커 = 적용 완료. 갱신이 실패하면 마커 없이 비0 — 다음 틱이 같은 요청을 다시 집는다).
...
- `none: …` → 아무것도 안 한다(조용). **이미 적용된 허용**(마커보다 앞선 문형)도 여기다 —
  그래야 같은 코멘트가 매 틱 warn 을 쌓지 않는다.
- stderr 에 `문형이 어긋난 '회차 허용:'` warn 이 뜨면 사람이 하이픈·빈 범위 등으로 적은 것이다
  (판정은 안 켜진다). 그 줄을 ④ Report warn 에 그대로 올려 사람이 다시 적게 하라.
- 비0 → warn 한 줄만 남기고 그 PR 의 검증은 평소대로 진행한다(허용을 못 읽은 것이 검증을
  막지는 않는다 — 못 읽으면 그 PR 은 기본 처방인 재발행으로 간다).
- 판정 규칙 셋(**줄 머리 또는 공백·em dash·`*` 뒤**에서 시작 — 따옴표·등호 바로 뒤는 인용 ·
  머신 코멘트(`<!-- bodat:worker -->`) 제외 · **인용(코드펜스·인라인 코드) 안은 신호가
  아니다**)의 SSOT 는 그 헬퍼다. 범위는 문형 뒤 줄 끝까지(굵게 표시 `**` 만 걷는다). 그래서 이 문서의 예시
...
1. `$SCRIPTS/reissue-pr.sh <repo> <pr> <issue>` **한 호출**이 전부를 한다(순서가 안전 계약):
   ...
   이전 → **원 이슈 닫기**(not planned) → **PR 닫기**(브랜치는 남긴다 — 새 워커가 참고한다).
   PR 이 **마지막**인 이유: 원 이슈 닫기가 실패해도 PR 이 `verify-eligible`(is:open) 큐에
   남아 다음 틱이 같은 PR 로 재진입한다(재심 2026-09-12 — PR 을 먼저 닫으면 재개 경로가 없다).
   **이번 회차의 실패 사유를 `--reason "<사유>"` 로 넘긴다**(위 재디스패치라면 bounce-comment
   에 줬을 그 문장 — `E2E 실패: <파일::테스트>`·`codex BLOCKER: …`·`결정적 CI 실패 — …`).
   상한 회차는 반송 코멘트를 남기지 않으므로 이걸 안 넘기면 새 이슈의 "현재 실패 사유" 절이
   **이전 회차**의 `재검증 실패:` 로 채워진다(E2E/CI 실패로 상한에 닿은 회차의 원인이 유실).
   검증자 절은 `<!-- bodat:worker -->` 마커가 든 `검증자 리뷰:`/`재검증 실패:`/`마감 검증:`
   코멘트 중 마지막 것이다 — 사람이 그 머리말을 인용한 코멘트는 후보가 아니다.
   라벨은 원 이슈에서 상속하고 `agent-ready` 를 붙인다 — `Epic #N` 줄과 P 는
   `$SCRIPTS/spinoff-inherit.sh`(#261) 가 준 값 그대로다(부모에 P 가 없으면 P2. 헬퍼 실패는
   exit 2 무쓰기). `spinoff` 는 **안 붙인다**(파생이 아니라 **재시도**다). 템플릿:
   `references/reissue.md`.
2. 종료 코드로 갈린다 — 0 성공 · **65 = 살아있는 회차 허용**(재발행 말고 위 재디스패치로
   가라, 범위는 stdout) · 2 조회 실패(**아무것도 쓰지 않았다**) · 3 발행·확인 실패
   (**아무것도 닫지 않았다** — 확인 = 새 이슈가 OPEN 이고 `agent-ready` 가 붙었나) ·
   4 발행 뒤 단계 실패(새 이슈는 있고 PR 은 열린 채다 — 다음 틱이 같은 PR 로 재진입해
   `<!-- reissued: #N -->` 마커를 보고 발행을 건너뛰되 **같은 확인을 다시 지난 뒤** 남은
   닫기(원 이슈·PR)만 이어간다 — 어느 단계에서 죽어도 마저 닫힌다.
   마커 코멘트 자체가 실패한 회차는 예외로 닫기를 계속한다 — 거기서 멈추면 다음 틱이 마커를
   못 보고 같은 이슈를 또 내기 때문이다. 그 창은 stderr warn 으로 드러난다).
...
3. 라벨 전이는 없다 — PR 이 CLOSED 라 `verify-eligible`(`is:open`)에서 자동으로 빠진다.
   `transition.sh` 를 따로 부르지 마라(닫힌 PR 의 라벨을 옮길 이유가 없다). 워크트리도
   지우지 않는다 — 새 워커는 새 이슈 번호로 자기 워크트리를 받고, 옛 `agent/issue-<구>`
   는 참고용으로 남는다(`cleanup-worktree.sh` 호출 금지).
...
- ② Pick 이 **새 후보를 집으면**(이번 PR 은 passed→flow:ready 로, redispatched/held→
  `verifying` 제거로, reissued→PR 이 CLOSED 라 `is:open` 필터로 이미 큐에서 빠졌다.
  flake_retry 만 flow:verify 가 남는데(unpick 결과) — 같은 PR 재선정 방지 위해 이 틱
  드레인에서는 **이번 틱에 이미 처리한 PR 번호를
  건너뛴다**) 그 PR 로 ③ 을 이어간다.
- ② Pick 후보가 **0이면**(또는 남은 게 이번 틱 처리분뿐이면) 드레인을 멈추고 ④ Report.

무한루프 방지: 각 반복은 큐를 최소 1 줄인다(passed→flow:ready 소멸·redispatched/held→
`verifying` 소멸 — 출구 전이가 뗀다·reissued→PR CLOSED 로 소멸). 같은 PR 이 두 번 집히면(flake_retry 반복 등) 그 PR 을 skip 하고
