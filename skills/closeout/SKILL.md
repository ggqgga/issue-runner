...
| ⓒ 대상 코드를 형제 PR 이 바꾸는 중이거나, 발행 시점에 `origin/<default>` 에 그 줄이 없다(`git grep` 으로 확인) | 형제 PR 이 열려 있으면 **그 PR 로**(반송해 리베이스), 이미 머지돼 사라졌으면 **기각** |
| ⓓ 사람이 정해야 하는 갈림길("둘 중 하나로") | **needs-human 코멘트** 한 줄 — 부모 에픽이 있으면 에픽에, 없으면 이 PR 에. 이슈를 만들지 않는다 |
| ⓔ 이 PR 밖 코드의 실제 결함(검증자가 P1 로 냈다가 범위 밖이라 WARN 으로 낮춘 것) · 워커 `follow-up:` 항목 | **이슈** — 아래 발행 명령으로 `references/spinoff-issue.md` 를 채워 agent-ready 이슈로 발행. 본문 둘째 줄 `Spinoff of PR #<pr> (issue #<부모>)` 필수 |
