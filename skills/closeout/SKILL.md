...
- 그런 코멘트가 하나도 없으면(PR 기준) → 보류가 없었다 → **`H`(이슈·PR 의 `needs-human`·
  `hold:*`)가 현재 없을 때만** 그대로 ② Pick. **`H` 가 있으면 무접촉(보류 유지)** — 사람이
...
- **`r` 없음 — 반송을 거치지 않은 새 커밋** → 먼저 **하류 활성 레인 소유**인지 본다(아래 멱등
  분기와 **같은 라벨 집합**, 정의는 거기 한 자리):
  - 라벨이 **하나라도 있으면** → 그 레인이 들고 있다 → `active` 무접촉.
    **해제 경로**: 그 레인의 완결 판정 → 해소(위와 같다).
  - **하나도 없으면** → 3) **모호와 같은 경로**로 사람에게 보낸다
...
- **예전엔 이 갈래가 해소였다** — "보류 이후 head 커밋이 더 늦으면 해소 → ② Pick" 이
  **상류에서 먼저 발화**해, 아래 멱등 분기가 세운 '착수≠완결' 규칙에 **도달조차 못 했다.**
  ...
  멱등 분기** 순으로만 읽어라.
- 이 순서의 **알려진 대가**: `(완결 없음, 새 커밋 있음, 마커 > 경계, 레인 라벨 없음)` 칸에서는
  착수 판정이 먼저 `active` 를 내어 아래 멱등 분기의 *전이만 재호출* 에 도달하지 않는다(격자
  8행). 도달 조건은 좁다 — 전이가 성사되면 `agent-ready` 가 워커 수명 내내 남으므로
  (`transition.sh` 전이 표: 어느 remove 칸에도 `agent-ready` 가 없다) "전이 실패 ∧ 그 뒤 사람이
  직접 커밋" 조합에서만 선다. 방향은 fail-closed(오머지 없음, 정지)이고 **해제 경로**는 #265
  정지 미러 warn 과 사람이다. 순서를 뒤집어 이 칸을 살리면 축① 이 다시 열리므로 뒤집지 마라.
...
연결 이슈가 **하류 활성 레인 라벨**을 현재 갖고 있는가.

**하류 활성 레인 라벨 = `agent-ready` ∨ `agent:claimed` ∨ `flow:verify` ∨ `flow:ready` ∨
`harvesting`** — `scripts/transition.sh` 의 전이 표에서 `closeout-redispatch` **이후**의
사다리가 이슈에 남기는 칸 **전부**다(`closeout-redispatch`→`agent-ready`, 디스패처
claim→`agent:claimed`, `handoff-verify`→`flow:verify`, `verify-pass`→`flow:ready`,
`closeout-pick`→`harvesting`). **워커 레인 두 라벨(`agent-ready`·`agent:claimed`)만 보면
안 된다(BLOCKER ①-a)**: `handoff-verify` 는 이슈에서 `agent:claimed` 를 **떼고**
...
- **하나라도 있다** → 전이가 섰다 → 하류 레인이 이 건을 들고 있다 → **이번 틱은 손대지 마라**
  ...
  **넓힌 값의 대가**를 함께 적는다 — 라벨 집합을 넓힌 만큼 "전이가 섰다" 가 더 자주 참이 되어
  *전이만 재호출* 갈래는 더 좁아진다. 그 방향은 fail-closed(살아있는 레인을 안 건드린다)이고,
  좁아진 만큼의 정체는 위 두 해제 경로가 본다.
- **하나도 없다** → 마커는 섰는데 **전이가 안 섰다**(마커-선행 순서에서 전이만 실패했거나
...
`attempt5` 열은 **직전 회차의 설계**(닫힌 PR #203 / 브랜치 `agent/issue-198`, head `683cdde2`)가
그 입력에서 하던 것이고, `뒤집힘` 열이 이번 회차가 새로 닫는 칸이다 — **축④**=검증자 BLOCKER ①
(살아있는 하류 레인을 걷어냄 · 값 없이 되돌리기), **축⑤**=검증자 BLOCKER ②(해소가 라벨 부재를
안 봄), **축⑥**=사전 리뷰가 찾은 **해제 경로 없는 칸**(경계 0 + 라벨만 · 반송 없는 새 커밋의
자기 순환 · `r ≤ h` 미정의). 축①②③ 은 직전 회차가 이미 닫은 칸이고, 그 열은 **10:11 반송
시점**의 상태다(그 회차가 고친 뒤의 값이 `want` 와 같다는 뜻이 아니라, 이 표가 그 축들의
회귀를 함께 못박는다는 뜻이다).

`h`=보류 경계 · `r`=재디스패치 마커 · `f`=완결 판정 코멘트(`머지 판정: ✅`·`마감 검증: ✅`) ·
`c`=head 커밋이 더 늦음 · `L`=이슈에 **하류 활성 레인 라벨**(`agent-ready`∪`agent:claimed`∪
`flow:verify`∪`flow:ready`∪`harvesting`) 현재 존재 · `H`=이슈·PR 에 `needs-human`·`hold:*`
현재 존재 · `D`=창 뒤 결정 코멘트 존재.

| # | 입력 | `origin/main` | `attempt5` | `want` | 뒤집힘 |
|---|---|---|---|---|---|
| 1 | `h` 없음, `H` 없음 | ② Pick | ② Pick | ② Pick | — |
| 2 | **`h` 없음**(경계 코멘트 0), **`H` 있음**(사람이 코멘트 없이 라벨만 붙임) | ② Pick | **② Pick**(라벨을 한 번도 안 읽는 출구) | **무접촉 · 보류 유지** | ✅ 축⑥ |
...
| 5 | `h`, `f` 있으나 **`f < r`**(마커보다 이른 ✅) | ② Pick | **해소 → ② Pick** | **`active`**(`max(h,r)` 뒤여야 해소) | ✅ 축① |
| 6 | `h`, `f` 아닌 `머지 판정: 🔄` 만 뒤에 있음 | ② Pick | **완결로 새어 해소** | **완결 아님** → 착수/멱등 분기로 | ✅ 축① |
| 7 | `h`, `c`(`f` 없음), **`r` 있음**, `L` 있음 | ② Pick | **② Pick**(상류가 먼저 발화해 '착수≠완결' 에 도달 못 함) | **`active` 무접촉** | ✅ 축① |
| 8 | `h`, `c`(`f` 없음), **`r` 있음**, **`L` 없음**(전이 실패 ∧ 사람 직접 커밋) | ② Pick | **② Pick** | **`active` 무접촉** — 착수 판정이 먼저다(순서 계약), 정체는 #265 warn 이 본다 | ✅ 축① |
| 9 | `h`, `c`(`f` 없음), **`r` 없음**, **`L` 있음** | ② Pick | **`active` 무접촉** | 같음(그 레인 소유 — 해제는 그 레인의 완결 판정) | — |
| 10 | `h`, `c`(`f` 없음), **`r` 없음**, **`L` 없음**(③-2 conflict 보류를 사람이 리베이스·push 하고 품) | ② Pick | **`active` 무접촉 → 자기 순환**(옛 ✅ 가 head 보다 일러 ①-b 가 영원히 `active`, 워커·verify 레인 모두 없음) | **3) 모호와 같은 경로** → `closeout-blocked --reason policy` | ✅ 축⑥ |
| 11 | `h`, **`pr-head-at.sh` exit 1**(값 없음), 이슈에 `flow:verify` 만 | ② Pick | **'읽어 내려가라' → 실측 갈래 → 전이 재호출**(fail-open) | **`BLOCKED: head 조회 실패` · 상태 불변 · 무접촉** | ✅ 축④ |
| 12 | `h`, `r > h`, `c` 없음, `L` 있음 | ② Pick | `active` 무접촉 | `active` 무접촉 | — |
| 13 | `h`, `r > h`, `c` 없음, `f` 없음, **`L` 전부 없음**(전이 실패·유실) | ② Pick | **`active` 무접촉 → 영구 정체**(워커가 영영 안 옴) | **마커 재발행 없이 전이만 재호출** · 또 실패면 `BLOCKED: 전이 실패 재디스패치` | ✅ 축② |
| 14 | `h`, `r > h`, `c` 없음, `f` 없음, 워커 두 라벨은 **없고** 이슈에 **`flow:verify` 있음**(`handoff-verify` 착지, 검증자 판정 전 창) | ② Pick | **전이 재호출 → 살아있는 검증 레인의 `flow:verify` 를 뗌** | **무접촉**(하류 활성 레인 소유) | ✅ 축④ |
...
| 22 | `h`, `pr-comments.sh` 조회 실패 | ② Pick | 모호(인용 면제) | 같음 | — |
