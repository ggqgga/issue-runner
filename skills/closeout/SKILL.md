...
| 단계 | 마커 | 재개 판정 |
|---|---|---|
| 1 검증 | PR 코멘트 `마감 검증:` | `✅` 면 1단계 건너뜀 · `⚠ 보류` 면 ③ 1단계 진입 전 재진입 규칙(#174) |
...
세 겹으로 막는다(셋 다 **증명되지 않으면 열지 않는다** 방향):
...
2. **반송 마커 안전망** — 반송 직후 아직 새 커밋이 없어 head 시각이 그대로인 창을 덮는다.
   ...
   시각만으로는 가릴 수 없다.
3. **보류 해소 대조 (#174)** — 최신 ✅ 를 그보다 **뒤에 달린** `마감 검증: ⚠ 보류` 가 가리면,
   사람이 그 뒤에 **사람 몫 사유 라벨**(`hold:policy`·`hold:conflict`)을 뗐음을
   `pr-hold-released-at.sh`(타임라인 `unlabeled` 이벤트, `--paginate` 전량)로 **증명**했을
   때만 ✅ 가 살아난다. 조회 실패·해제 이벤트 없음·동초는 전부 `active`(무접촉)다 —
   `held` 가 아닌 이유는 `held` 가 스윕에 `closeout-blocked`(= `needs-human` **부착**)를
   시켜 사람이 방금 뗀 보류를 루프가 다시 붙이는 경로가 되기 때문이다(#151).
   **`needs-human`·`hold:ladder` 의 제거는 세지 않는다** — `resume-sweep.sh` 가 사다리
   자동 재개로 그 둘을 **기계가** 떼기 때문이다(:159,:299). 그걸 세면 기계 동작이 "사람이
   결정했다" 는 증명으로 둔갑한다(이 이슈 「걸러선 안 되는 것」 1항의 거울상). **`hold:policy`
   에는 또 하나의 기계 경로가 있다 (#174 재작업, 반송 P1)** — 루프 SKILL.md
   `policy_review_due` 절의 **디스패처 자체 재심**이다. 이건 "반송"(최신 판정을 `🔄` 로
   되돌리는 재검증)이 아니라, 이슈의 hold-note 질문에 루프가 스스로 답하고 `transition.sh
   verify-redispatch` 로 `hold:policy` 를 떼는 것뿐이다 — 가려진 ✅ 는 그대로 남는다. 그래서
   `pr-hold-released-at.sh` 는 `hold:policy` 의 각 해제 이벤트마다 그 직전 `<!-- hold-note:
   policy -->` 코멘트 이후에 `<!-- policy-review: resumed -->`/`kept` 마커가 있는지
   대조하고, 있으면 그 해제를 **후보에서 뺀다**(기계 재심 ≠ 사람 결정). 이 대조가 성립하려면
   `policy_review_due` 절차가 재심 답변 코멘트를 **PR 에도 미러**해야 한다(이슈만 남기면 이
   헬퍼가 못 본다) — issue-runner SKILL.md 의 그 절이 이를 명시한다. 사람 대기
   여부 자체는 위 **대상 필터의 `needs-human` 제외**가 이미 소유한다 — 두 신호를 한 값에
   겹쳐 담지 않는다. 보류가 최신 ✅ 보다 **앞**이면 이미 지나간 보류라 아무것도 가리지
   않는다(반송 후 재완결이 영구 억제되지 않게). 그리고 **`(보류 코멘트, 해제]` 창 안**의
   사람 코멘트는 그 해제로 답해진 것이므로 "미해결 사람 코멘트" 로도 세지 않는다(운영자
   결정문이 큐를 막던 실측 경로). 창 **밖**은 종전대로 미해결이다 — 해제 뒤의 새 질문도,
   그 보류와 무관한 옛 미결도 삼키지 않는다. ★알려진 좁힘(#174 재작업 [P2], 미해결)★ —
   이 창의 닫는 쪽 경계(`hold_released_at`)는 **전역 최신** 해제이지 "그 보류에 짝지어지는"
   해제가 아니다. 원 보류가 풀린 뒤 사람이 새 질문을 남기고, 그 뒤 **무관한**(그러나 진짜
   사람이 푼) `hold:policy`/`hold:conflict` 사이클이 또 풀리면 그 새 질문까지 소급 면제될
   수 있다 — 이 회차는 위 기계 재심 배제(P1, 가장 흔한 촉발 경로)만 닫았다. 근본 수정은
   "해제를 그 보류에 짝짓는" `pr-hold-released-at.sh` 인터페이스 변경이 필요해 범위를
   좁혔다(별도 이슈로 분리하는 편을 권한다).

세 겹 모두 **코멘트를 전부 봤다**는 전제 위에 선다. `gh pr view --json comments` 는
...
| `active` | 진행 중·버퍼 미도달·우리 형상 아님, 또는 **✅ 의 신선도를 증명 못 함**(✅ 가 head 커밋보다 이르거나 두 시각 중 하나를 못 얻음, #171), 또는 **그 ✅ 를 뒤에 달린 `마감 검증: ⚠ 보류` 가 가리는데 사람의 해제가 그보다 뒤임을 증명 못 함**(#174) | **무접촉**(다음 틱). **여기에 `closeout-blocked` 를 걸지 마라** — 사람이 방금 뗀 보류를 루프가 다시 붙이게 된다(#151). |
...
**1단계 진입 전 — 보류 해제 재진입 (#174).** 이 PR 에 `마감 검증: ⚠ 보류` 가 이미 있고 그
**뒤에** 사람이 사람 몫 사유 라벨(`hold:policy`·`hold:conflict`)을 뗐다면 — 그래서 이 PR 이 다시 후보로 올라왔다
(`closeout-eligible.sh` → `pr-hold-released-at.sh` 가 해제 시각이 그 보류 코멘트보다 늦음을
증명했다) — **게이트를 그대로 다시 돌리지 마라.** 코드가 안 바뀌었으니 같은 게이트가 같은
`[P1]` 을 다시 내고 → 다시 보류 → 사람이 또 푼다 → 무한 루프다. 그 BLOCKER 의 근거 자체가
사람에게 **사실오인으로 기각**된 사례가 있다(실측 BodaT PR #4922 — 재실행은 그 오판을
재생산할 뿐이다).

`$SCRIPTS/pr-comments.sh <repo> <pr>` 로 코멘트를 읽고, **해제 시각 언저리의 사람 코멘트**
(머신 마커 `<!-- bodat:worker -->` 가 없는 것 — 운영자는 결정문을 쓰고 곧바로 라벨을 뗀다)를
근거로 세 갈래로 나눈다:

- **결정문이 직전 `⚠ 보류` 가 물은 질문에 답하고 "원안 그대로 머지" 류다** → 그 발견을
  **재판정하지 않는다.** 1단계 게이트를 건너뛰고 근거를 남긴 뒤 2단계로 간다:
  `gh pr comment <pr> --repo <repo> --body "마감 검증: ✅ 사람 판정 <요지 한 줄> — 원안 그대로 머지 (<작성자>·<코멘트 시각>)
  <!-- bodat:worker -->"`
  — 자동 머지의 권위가 **사람 결정**임이 PR 에 보여야 한다(누가·언제·무엇을 정했는지).
  이건 거짓 BLOCKER 반전이므로 아래 lessons 기록도 함께 남긴다.
- **해제 뒤 새 커밋이 있다** → 사람이 방향을 정해 주고 **워커가 고치는 중**이다(반송 레인 —
  issue-runner #166 이 그 경우였다). closeout 은 머지로 가지 않는다. 손대지 말고 워커의 새
  판정을 기다려라. (이 갈래는 `finish-classify` 의 #171 head 대조가 `active` 로 내므로 보통
  후보에도 안 온다 — 그래도 왔다면 여기서 멈춘다.)
- **결정문을 못 찾겠다 · 다른 주제다 · 모호하다** → **종전대로 게이트를 돌린다**(fail-closed).
  운영자가 `사람 결정:` 같은 접두어를 매번 쓴다고 가정해 파싱하지 마라 — 안 썼을 때 조용히
  게이트를 건너뛰거나 반대로 영영 못 읽는다. 못 찾으면 재실행이 기본값이다.

**`hold:*` 를 루프가 스스로 떼지 마라.** 이 규칙은 **사람이 뗀 것을 읽는** 일이다 — 루프가
자기 보류를 자기가 푸는 경로가 생기면 사람 게이트가 무의미해진다(사다리의 `hold:ladder`
자동 재개는 별개 축이고 그 범위를 넓히지 않는다).
