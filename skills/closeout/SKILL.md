...
- **실장비 줄은 접두 표식 `[칸 ③]` 필수** — `- [ ] [칸 ③] <동작>` 형태로 적는다.
  ...
  칸 ①② 실패로 이월되어 줄에 "TEST 워커" 라는 말이 없더라도 **밟는 칸이 ③ 이면 표식을
...
- **실장비 항목이 남아 있으면 green 이어도 닫지 않는다 (칸 ③ 은 크롬이 못 밟는다).**
  ...
  **표식이 없는 줄은 크롬 대상이다** — 해석으로 실장비 승격하지 마라(그 순간 실장비를 세는
  계산기가 둘이 된다). 다만 크롬으로 도저히 대조할 수 없으면 pass 로 찍지 말고 **fail 로
  두어** 아래 fail 갈래(후속 이슈 발행)로 보낸다 — 거짓 초록보다 후속 이슈가 낫다.
  4단계 산출에서 표식이 빠진 것으로 보이면 그 사실을 배포 이슈 코멘트에 한 줄로 남긴다.
