...
- **다르면 모호**다(시각으로 tie-break 하지 마라 — 초 단위라 못 가린다).
...
| 방향 | 읽는 신호 | 조치 |
|---|---|---|
| **시정** | "판정이 맞다"·"좁힌다/고친다/바꾼다"·구현 지침·추가 테스트 요구 등 **코드를 바꾸라는 문장이 하나라도** 있다 | `$SCRIPTS/transition.sh closeout-redispatch <repo> <issue> <pr>` + PR 에 `gh pr comment <pr> --repo <repo> --body "재디스패치: #<이슈> — 사람 재심이 시정 방향(<결정문 인용 한 줄>)`⏎`<!-- bodat:worker -->"`. 이 마커가 `BOUNCE_MARKERS` 에 있어 이후 틱의 `closeout-eligible.sh` 가 자동 배제한다. **② Pick 하지 않는다.** — **연결 이슈가 없으면** 이 전이는 부를 수 없다(`closeout-redispatch` 는 이슈 번호가 필수: 되돌릴 `agent-ready` 가 없다). 반송할 레인이 없으므로 아래 **모호 행과 같은 조치**로 사람에게 보내고 `--note` 에 `시정 방향인데 연결 이슈가 없어 반송 불가` 를 적는다. |
