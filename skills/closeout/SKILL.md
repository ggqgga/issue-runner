...
- **출력이 없으면(exit 1 — 판정 실패)** → `active` 취급 **무접촉**(fail-closed — 반송되지
  않았음을 *증명*했을 때만 연다, #171 과 같은 방향).
- `bounced` 면 → **무접촉으로 끝내지 말고 아래 2) 의 `$SCRIPTS/finish-classify.sh` 로
  분류한다**(#206). 단, **재디스패치 갈래만 연다**:
  - `stale_reverify` → **재디스패치**(2) 표의 그 행과 같은 조치 — `closeout-redispatch`
    전이 + 멱등 마커). 반송 회차 워커가 커밋까지 하고 ✅ 직전에 죽은 형상이다.
  - `stale_inline` → **여기서는 입양(머지)이 아니라 같은 재디스패치**다. 반송된 PR 에
    남아 있는 `검증자 리뷰: CLEAN` 은 **반송 이전 회차**의 것일 수 있어, 입양하면 방금
    반려된 코드를 머지한다(#196 이 막은 바로 그 방향) — 그래서 입양은 절대 열지 않는다.
    그렇다고 무접촉으로 두면 "교체 워커가 고쳐 커밋하고 ✅ 직전에 죽은" 같은 정체가
    `stale_inline` 칸에 그대로 남는다(이 이슈가 없애려는 바로 그 좌초의 형제). 두 요구를
    함께 만족시키는 조치는 하나뿐이다: **머지하지 않고 재디스패치**.
  - 그 밖의 모든 출력(`active`·`done_verdict`·`held`) → **무접촉**. `done_verdict` 도
    여기선 무접촉이다 — ✅ 정상 경로는 `closeout-eligible.sh` 가 자기 반송 안전망과
    함께 소유한다.

왜 필요한가(#196 실측: bodat PR #5009 / 이슈 #4973): 반송된 PR 은 `머지 판정: ✅` 가
없어 `closeout-eligible.sh` 목록에 애초에 안 뜬다. 반송 직후에는 `transition.sh` 가 단계
...
| `active` | 진행 중·버퍼 미도달·우리 형상 아님, 또는 **✅ 의 신선도를 증명 못 함**(✅ 가 head 커밋보다 이르거나 두 시각 중 하나를 못 얻음, #171), 또는 **진행 증거가 있음**(커밋이 `STALL_MIN` 이내이거나 head SHA 의 CI 티켓이 큐에 살아 있음 — 또는 그 판정 자체가 불가 — queue.log 를 못 읽음·head 조회 실패(`unknown`≠`none`), #206 · `progress-evidence.sh`). **위 1) 의 CONFLICTING PR 중 `bounce-state.sh` 가 판정 실패(exit 1)인 것도 여기로 떨어진다**(#196) — 단계 라벨이 비어 있어도 그건 좌초가 아니라 워커 레인 소유다 | **무접촉**(다음 틱). |
