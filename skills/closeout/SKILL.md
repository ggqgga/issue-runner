...
|---|---|---|
| `ok` | 반송 마커가 없거나, 최신 반송 마커 뒤 마지막 판정이 `머지 판정: ✅` | **마커표 그대로** — 끝난 단계를 건너뛰고 중단 지점부터. 단 반송 마커가 원장에 **있으면** 1단계 마커는 **그 반송보다 뒤**의 `마감 검증:` 코멘트만 센다(반송 앞의 것은 반송 사유가 된 옛 head 에 대한 판정이다 — ✅ 에 적용하는 #171 규칙을 1단계 마커에도 그대로) |
...
- ⓐ **구현으로 닫히는 결함 → 워커 레인 반송**(실측 2회 — 이 갈래에 코멘트 채널이 없어
  ...
  **곧바로 `$SCRIPTS/transition.sh closeout-pick <repo> <issue> <pr>` 를 한 번 더 걸어 PR 과
  ...
  `마감 검증:` 을 스스로 남기지 않지만 **이전 회차가 남긴 마커가 이미 있을 수 있고**, 그러면
  마커표는 "있으면 1단계 건너뜀" 대로 ③-1 을 건너뛰어 방금 BLOCKER 를 낸 head 를 2단계 머지
  게이트로 보낸다(그 게이트 조건은 반송 직전 상태 그대로 전부 참이다). 그래서 ① Reconcile 의
