...
| 단계 | 마커 | 재개 판정 |
|---|---|---|
| 1 검증 | PR 코멘트 `마감 검증: ✅`(접두 — `마감 검증: ✅ 기각 승계` 도 이 접두라 함께 걸린다) | 있으면 1단계 건너뜀. **`마감 검증: ⚠ 보류` 는 매칭하지 않는다** — 낡은 보류 마커만 남았으면 1단계(③-1)를 다시 돈다(#198: 접두를 `마감 검증:` 로만 두면 보류 코멘트도 걸려, 보류 뒤 새 커밋이 재검증 없이 머지 게이트로 새는 창이 생긴다) |
...
|---|---|---|
| `done_verdict` | 최신 `머지 판정: ✅` **이고 그 판정이 현재 head 커밋 이후임이 증명됨**(#171) | eligible.sh 정상 경로가 처리 — 스윕은 skip. 단 그 정상 경로도 **①-c 방향 판정을 먼저 통과**해야 ② Pick 이 집는다(#198 — 사람이 "고쳐라" 라고 푼 보류는 ✅ 가 신선해 보여도 머지 후보가 아니다). 그리고 **미해소 `마감 검증: ⚠ 보류` 가 있으면 skip 하지 말고 ①-c 로 보낸다** — 사람이 결정문을 PR 에 마커 없이 쓰면 eligible 의 `unresolved` 게이트(#72)가 그 PR 을 후보에서 빼서 ①-c 가 아예 안 돌기 때문이다(조용한 정체) |
...
| `held` | 최신 `머지 판정: ⚠ 보류` (워커 명시 보류) | **해제가 이미 일어났으면**(①-c 2) 의 논리곱이 참 — 창 뒤 결정 코멘트 ∧ `needs-human`·`hold:*` 현재 부재) **①-c 로 보낸다**(시정→`closeout-redispatch` 반송, 모호→`closeout-blocked`, 기각→② Pick) — 그 전에 `closeout-blocked` 를 다시 부르면 사람이 방금 푼 보류를 **매 틱 되살린다**(#225 가 못박은 자리 — 진입만 보고 해제 경로를 안 재면 루프가 사람과 싸운다). **아직 해제 전이면 needs-human** — `$SCRIPTS/transition.sh closeout-blocked <repo> <issue|-> <pr> --reason policy --note "<질문 한 줄>"` (PR 과 연결 이슈 **양쪽**에 `needs-human` + `hold:policy` 부착 + 단계 라벨 정리 — 연결 이슈가 없어도 PR 에 사람 신호가 남는다), closeout 무접촉(자동 진행 안 함). |
| `active` | 진행 중·버퍼 미도달·우리 형상 아님, 또는 **✅ 의 신선도를 증명 못 함**(✅ 가 head 커밋보다 이르거나 두 시각 중 하나를 못 얻음, #171). (CONFLICTING 이든 MERGEABLE 이든 반송 회차 진행 중이거나 판정 실패면 1) 게이트에서 이미 `active` 로 걸러져 여기까지 오지 않는다 — #218, #196) **미해소 `마감 검증: ⚠ 보류` 가 있는 PR 은 여기서 끝내지 마라**(#198) — ①-c 가 해제 방향을 읽어 *시정*이면 `active`(무접촉)가 아니라 **반송**(`closeout-redispatch`)이고, 방향이 *모호*하면 `closeout-blocked`(사람)다. 무접촉으로 두면 다음 틱에 그대로 다시 후보가 된다 | **미해소 `마감 검증: ⚠ 보류` 가 있으면 ①-c 로**(시정→`closeout-redispatch` 반송, 모호→`closeout-blocked`, 기각→② Pick) — **없으면 무접촉**(다음 틱). |
...
있으니 인자로 붙이지 마라). 조회가 exit 1 이면 **증명 실패**이므로 통과시키지 말고 아래 3) 의
*모호* 로 간다.
...
- 그런 코멘트가 하나도 없으면(PR 기준) → 보류가 없었다 → 그대로 ② Pick.

**해소 판정 — 새 완결 판정만이 해소다(#198 10:11/P1 축①).** PR 배열에서
**완결 판정 코멘트의 인덱스 > `max(보류 경계, 재디스패치 마커)`** 이면(= 보류 **그리고**
마지막 반송 **뒤에** 새 완결 판정이 섰다) → **해소** → 그대로 ② Pick(③-1 은 정상대로 돈다).
비교 기준이 보류 경계 하나가 아니라 `max(보류 경계, 재디스패치 마커)` 인 이유: 마커보다
**이른** ✅ 는 반송 이전 코드에 대한 판정이라, 보류 경계만 넘으면 해소로 세는 순간 같은
구멍이 옆문으로 다시 열린다.
...
**새 커밋은 해소가 아니라 착수 신호다(#198 재심/P1 + 10:11/P1 축①).**
`$SCRIPTS/pr-head-at.sh <repo> <pr>` 가 낸 head 커밋 시각이 `max(보류 경계, 재디스패치 마커)`
코멘트의 `createdAt` 보다 **늦으면**, 그건 누군가 **착수**했다는 증거일 뿐 **완결**했다는
증거가 아니다 → **`active`(무접촉)** 로 두고 **이번 틱은 손대지 마라**. 마커도 다시 쓰지 말고
`$SCRIPTS/transition.sh closeout-redispatch` 도 다시 부르지 마라. 이건 `stale_reverify` 행이
쓰는 술어와 **같은 것**이고(head 커밋이 신선하면 코멘트가 낡았어도 `active` 로 떨어져 살아있는
attempt N+1 워커를 오분류하지 않는다) 새 헬퍼를 만들지 않고 이미 있는 `pr-head-at.sh` 를
그대로 재사용한다.
...
- **예전엔 이 갈래가 해소였다** — "보류 이후 head 커밋이 더 늦으면 해소 → ② Pick" 이
  **상류에서 먼저 발화**해, 아래 멱등 분기가 세운 '착수≠완결' 규칙에 **도달조차 못 했다.**
  ...
  멱등 분기** 순으로만 읽어라.
- 대기를 끝내는 조건은 위 **해소 판정 하나뿐**이다. 새 완결 판정이 서면 `finish-classify` 가
  이 PR 을 독립적으로 `done_verdict` 로 분류하므로(①-b `done_verdict` 행) ①-c 가 정상
  재진입해 해소 경로로 합류한다 — 새 판정 기준을 만들지 않는다. 완결 판정이 영영 안 서면
  그건 정지이고, `loop-status.sh` 정지 미러 warn(#265)이 보이게 할 자리다.
- `pr-head-at.sh` 가 exit 1(무출력)이면 "새 커밋 없음" 으로 읽지 마라 — **못 얻은 것**이다.
  해소를 증명하지 못했으니 **미해소**로 두고 아래로 간다(fail-closed — ①-b 와 같은 방향).

**이미 반송됐으면 다시 반송하지 않는다 — 단 '마커가 있다' 가 아니라 '전이가 섰다' 로 가른다
(멱등, #198 반송3/P1 + 10:11/P1 축②).** **재디스패치 마커 인덱스 > 보류 경계** 이면(이 보류에
대한 반송이 이미 나갔다) 마커만 보고 무접촉하지 말고 **전이 효과 실측**을 한 번 한다 —
연결 이슈에 워커 레인 라벨(`agent-ready` 또는 `agent:claimed`)이 **현재 있는가.**

- **있다** → 전이가 섰다 → 워커 레인이 이 건을 들고 있다 → **이번 틱은 손대지 마라**
  ...
  재호출 구간이었다).
- **둘 다 없다** → 마커는 섰는데 **전이가 안 섰다**(마커-선행 순서에서 전이만 실패했거나
  나중에 되돌려졌다) → 그대로 두면 이슈가 `agent-ready` 를 영영 못 돌려받아 워커가 안 온다
  (영구 정체). 마커는 이미 서 있으니 **다시 쓰지 말고 전이만 재호출한다** —
...
- **순서가 계약이다** — 위 **해소 판정이 이 분기보다 먼저** 온다. 워커가 완결해
  `머지 판정: ✅` 를 찍고 레인 라벨이 정리된 뒤에 이 분기가 먼저 돌면 "라벨 없음 = 전이 실패"
  로 오독해 다 끝난 PR 을 다시 반송한다. `max(보류 경계, 재디스패치 마커)` 뒤의 완결 판정이
  먼저 걸러 그 오독을 막는다.
...
- ⓑ 가 거짓(**라벨이 아직 있다**) → 사람이 아직 들고 있다 → **무접촉**. 전이를 다시 부르지
  마라(이미 보류 상태라 아무것도 바꾸지 않는 호출이다). 결정문이 이미 달려 있어도 마찬가지다 —
  라벨이 남아 있으면 사람이 아직 안 놓은 것이다.
...
**단, 위 모호 행의 '결정 코멘트 부재'·'코멘트 조회 실패' 두 갈래는 이 인용 의무에서
면제한다(#198 반송3/P2)** — 그 두 경우엔 애초에 인용할 문장이 없어, 인용을 강제하면
무인 워커가 지어낸 인용을 채우거나 보류를 건너뛰게 된다. 그 갈래에서는 인용 자리에
진단 문구를 대신 남긴다: 결정 코멘트 자체가 없으면 `결정문 없음`, `pr-comments.sh` 조회가
exit 1 이면 `코멘트 조회 실패: <에러 한 줄>`.
...
`closeout-eligible.sh` 를 통과해 곧장 ② Pick 된다 — 그래서 main 열은 전 행이 `② Pick` 이고,
좁힘의 실효는 **`attempt5`(이 브랜치의 10:11 반송 시점 상태) → `want`** 대조에서 읽어야 한다.
`뒤집힘` 열이 이번 회차가 닫는 칸이다.

`h`=보류 경계 · `r`=재디스패치 마커 · `f`=완결 판정 코멘트(`머지 판정: ✅`·`마감 검증: ✅`) ·
`c`=head 커밋이 더 늦음 · `L`=이슈에 워커 레인 라벨(`agent-ready`∪`agent:claimed`) 현재 존재 ·
`H`=이슈·PR 에 `needs-human`·`hold:*` 현재 존재 · `D`=창 뒤 결정 코멘트 존재.

| # | 입력 | `origin/main` | `attempt5` | `want` | 뒤집힘 |
|---|---|---|---|---|---|
| 1 | `h` 없음 | ② Pick | ② Pick | ② Pick | — |
| 2 | `h`, `f > max(h,r)` | ② Pick | ② Pick | ② Pick | — |
| 3 | `h`, `c` 만(`f` 없음), `r` 없음 | ② Pick | **② Pick** | **`active`** | ✅ 축① |
| 4 | `h`, `r > h`, `c`(마커 뒤 새 커밋), `f` 없음, `L` 있음 | ② Pick | **② Pick**(상류 ⑵ 가 먼저 발화해 '착수≠완결' 에 도달 못 함) | **`active` 무접촉** | ✅ 축① |
| 5 | `h`, `r > h`, `c` 없음, `L` 있음 | ② Pick | `active` 무접촉 | `active` 무접촉 | — |
| 6 | `h`, `r > h`, `f` 없음, **`L` 없음**(전이 실패·유실) | ② Pick | **`active` 무접촉 → 영구 정체**(워커가 영영 안 옴) | **마커 재발행 없이 전이만 재호출** · 또 실패면 `BLOCKED: 전이 실패 재디스패치` | ✅ 축② |
| 7 | `h`, 마커 posting 실패(전이 미호출) | ② Pick | `BLOCKED` · 상태 불변 · 다음 틱 재시도 | 같음 | — |
| 8 | `h`, `r` 없음, `D` 있음, `H` 없음 | ② Pick | 3) 방향 판정 | 3) 방향 판정 | — |
| 9 | `h`, `r` 없음, `D` 있음, **`H` 있음**(라벨 아직) | ② Pick | **3) 방향 판정 진행**(기각이면 ② Pick) | **보류 유지 · 무접촉** | ✅ 축③ |
| 10 | `h`, `r` 없음, **`D` 없음**, `H` 없음 | ② Pick | 모호 → `closeout-blocked` | 같음(= 보류 유지) · 정체는 #265 warn 이 보인다 | — |
| 11 | `h`, 결정문이 **이슈에만** | ② Pick | 있는 쪽 방향 채택 | 같음 | — |
| 12 | `h`, PR·이슈 결정문 방향이 **다름** | ② Pick | 모호 | 같음 | — |
| 13 | `h`, `pr-comments.sh` 조회 실패 | ② Pick | 모호(인용 면제) | 같음 | — |
| 14 | `h`, `f` 있으나 **`f < r`**(마커보다 이른 ✅) | ② Pick | **해소 → ② Pick** | **`active`**(`max(h,r)` 뒤여야 해소) | ✅ 축① |
| 15 | 워커가 찍은 `머지 판정: ⚠ 보류` 의 해제 | ② Pick | **경계로 안 셈 → ② Pick** | 경계로 세고 → 2) 논리곱 → 3) 방향 판정 | ✅ 축③ |
| 16 | `h`, `f` 아닌 `머지 판정: 🔄` 만 뒤에 있음 | ② Pick | **완결로 새어 해소** | **완결 아님** → `active`/멱등 분기 | ✅ 축① |
