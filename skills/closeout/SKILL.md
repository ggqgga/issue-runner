...
세 겹으로 막는다(모두 **증명되지 않으면 열지 않는다** 방향):
...
2. **반송 마커 안전망** — 반송 직후 아직 새 커밋이 없어 head 시각이 그대로인 창을 덮는다.
   ...
   시각만으로는 가릴 수 없다.
3. **보류 해소 대조 (#174, 4회차 — 에피소드 키)** — 최신 ✅ 를 그보다 **뒤에 달린**
   `마감 검증: ⚠ 보류` 가 가리면, 사람이 그 뒤에 **사람 몫 사유 라벨**(`hold:policy`·
   `hold:conflict`)을 뗐음을 `pr-hold-released-at.sh`(타임라인 `labeled`/`unlabeled`
   이벤트, `--paginate` 전량, 3번째 인자로 그 `⚠ 보류` 코멘트 시각을 앵커로 넘긴다)로
   **증명**했을 때만 ✅ 가 살아난다. 조회 실패·해제 이벤트 없음·동초는 전부
   `active`(무접촉)다 — `held` 가 아닌 이유는 `held` 가 스윕에 `closeout-blocked`(=
   `needs-human` **부착**)를 시켜 사람이 방금 뗀 보류를 루프가 다시 붙이는 경로가
   되기 때문이다(#151). 위 `hold:*` 접두 필터(#242, ②Pick 대상 필터)는 **현재** 라벨
   상태만 보므로, 이 항목이 보는 **과거** 해제 이력(아래 면제 창 경계)과는 서로 다른
   질문이라 겹치지 않는다.

   3라운드가 "이 `unlabeled` 가 사람 해제인가" 를 **라벨 이름별로** 판정하다 매 라운드
   새 기계 제거 경로를 놓쳤다(1: `policy_review_due` 재심, 2: `kept` 마커 방향 반전,
   3: `transition.sh` 의 `hold_others()` — 사유를 바꿔 (재)보류할 때 나머지 두
   `hold:*` 를 **같은 API 호출로** 뗀다, 그중 `hold:conflict` 축은 못 걸렀다). 그래서
   4회차(사람 결정 B, #186 합류)는 라벨 이름을 전혀 모른 채 동작하는 **구조적 두
   규칙**으로 바꿨다(하나라도 걸리면 기계로 본다, 순서 없음):
   - **규칙 1(사유 교체)** — `unlabeled(hold:R)` 과 **같은 시각**에 `labeled(hold:R')`
     (R'≠R, 아무 `hold:*`) 가 있으면 사유 교체지 해제가 아니다. `hold_others()` 는
     새 사유를 붙이며 나머지를 같은 `gh issue edit` 호출로 떼므로 타임라인에 동초로
     찍힌다 — 사람이 라벨 하나를 손으로 떼는 동작은 이 짝을 만들지 않는다. 라벨명을
     안 보므로 `hold:conflict`·`hold:ladder`·앞으로 생길 `hold:<새사유>` 모두 코드
     변경 없이 덮인다.
   - **규칙 2(재심 마커 에피소드)** — 기존 `hold:policy` 전용 로직을 사유로
     매개변수화했다: 해제 직전 마지막 `<!-- hold-note: R -->` 이후에 `<!--
     R-review: resumed -->` 가 있으면 기계 해제(`kept` 는 여전히 라벨을 안 건드리므로
     제외). 이 마커 관용구를 쓰는 자동 절차는 지금 `policy_review_due` 하나뿐이라
     `conflict` 축엔 사실상 늘 "증거 없음 → 통과" 로 떨어진다 — 증거 없이 배제하면
     과잉 배제다(attempt 1 이 이미 세운 원칙).

   `hold:ladder` 는 규칙 2 로도 못 거른다(사다리 자동재개가 PR 코멘트에 마커를 안
   남긴다 — 마커는 이슈 전용) — 그래서 애초에 "release-candidate" 허용목록
   (`hold:policy`·`hold:conflict`)에 안 넣는다(사다리 재개는 항상 기계, README).
   보류가 최신 ✅ 보다 **앞**이면 이미 지나간 보류라 아무것도 가리지 않는다(반송 후
   재완결이 영구 억제되지 않게).

   **에피소드 스코프 해제(#186 합류)** — `pr-hold-released-at.sh` 는 앵커(판정 대상
   `⚠ 보류` 코멘트 시각)를 받으면 "그 시각 **이후 첫** 사람 해제" 를 낸다(전역 최신이
   아니라). 앵커 없이 2-인자로 부르면 종전처럼 전역 최신(단일 보류 사이클에선 값이
   같다). 이게 없으면: 원 보류가 풀린 뒤 사람이 새 질문을 남기고, 그 뒤 **무관한**
   `hold:policy`/`hold:conflict` 사이클이 또 풀렸을 때 그 최신 해제로 창이 전진해
   새 질문까지 소급 면제된다(#186 실측) — 앵커가 "이 보류를 닫은 해제" 로 창을
   좁혀 막는다. 그리고 **`(보류 코멘트, 해제]` 창 안**의 사람 코멘트는 그 해제로
   답해진 것이므로 "미해결 사람 코멘트" 로도 세지 않는다(운영자 결정문이 큐를 막던
   실측 경로). 창 **밖**은 종전대로 미해결이다 — 해제 뒤의 새 질문도, 그 보류와
   무관한 옛 미결도 삼키지 않는다.

세 겹 모두 **코멘트를 전부 봤다**는 전제 위에 선다. `gh pr view --json comments` 는
페이지네이션 없이 **첫 100건**만 주므로 세 헬퍼는 그 경로를 쓰지 않고 `pr-comments.sh`
...
| `active` | 진행 중·버퍼 미도달·우리 형상 아님, 또는 **✅ 의 신선도를 증명 못 함**(✅ 가 head 커밋보다 이르거나 두 시각 중 하나를 못 얻음, #171), 또는 **그 ✅ 를 뒤에 달린 `마감 검증: ⚠ 보류` 가 가리는데 사람의 해제가 그보다 뒤임을 증명 못 함**(#174). **위 1) 의 CONFLICTING PR 도 `bounce-state.sh` 가 `ok` 가 아니면(반송 회차 진행 중이거나 판정 실패) 여기로 떨어진다**(#196) — 단계 라벨이 비어 있어도 그건 좌초가 아니라 워커 레인 소유다 | **무접촉**(다음 틱). **여기에 `closeout-blocked` 를 걸지 마라** — 사람이 방금 뗀 보류를 루프가 다시 붙이게 된다(#151). |
