...
2. **반송 마커 안전망** — 반송 직후 아직 새 커밋이 없어 head 시각이 그대로인 창을 덮는다.
   ...
   + 낱말 경계** — 콜론을 리터럴로 요구하지 않고 뒤에 무엇이 오든 잡되(#212), 마커 바로
   뒤에 한글이 이어지면(`재디스패치가 …`) 조사가 붙은 평범한 문장이라 반송이 아니다(#221).
   새 반송 어휘가
...
| `active` | 진행 중·버퍼 미도달·우리 형상 아님, 또는 **✅ 의 신선도를 증명 못 함**(✅ 가 head 커밋보다 이르거나 두 시각 중 하나를 못 얻음, #171). **위 1) 의 CONFLICTING PR 도 `bounce-state.sh` 가 `ok` 가 아니면(반송 회차 진행 중이거나 판정 실패) 여기로 떨어진다**(#196) — 단계 라벨이 비어 있어도 그건 좌초가 아니라 워커 레인 소유다. **미해소 `마감 검증: ⚠ 보류` 가 있는 PR 은 여기서 끝내지 마라**(#198) — ①-c 가 해제 방향을 읽어 *시정*이면 `active`(무접촉)가 아니라 **반송**(`closeout-redispatch`)이고, 방향이 *모호*하면 `closeout-blocked`(사람)다. 무접촉으로 두면 다음 틱에 그대로 다시 후보가 된다 | **미해소 `마감 검증: ⚠ 보류` 가 있으면 ①-c 로**(시정→`closeout-redispatch` 반송, 모호→`closeout-blocked`, 기각→② Pick) — **없으면 무접촉**(다음 틱). |
...
- **보류 경계** = `마감 검증: ⚠ 보류` 로 시작하거나 `<!-- hold-note: ` 를 담은 코멘트의
  **마지막 매칭 인덱스**. `createdAt` 이 아니라 **코멘트 배열 인덱스**로 잰다 — GitHub 코멘트
  시각은 초 단위라 동초에 달린 코멘트의 선후를 못 가린다(`closeout-eligible.sh` 의 반송 마커
  안전망이 쓰는 관용구 그대로).
...
- **해소 판정**: PR 배열에서 그 인덱스 **뒤에** ⑴ `마감 검증: ✅` 로 시작하는 코멘트가 있거나
  ⑵ `$SCRIPTS/pr-head-at.sh <repo> <pr>` 가 낸 head 커밋 시각이 그 보류 코멘트의 `createdAt`
  보다 **늦으면**(= 보류 이후 새 커밋) → 해소 → 그대로 ② Pick(③-1 은 정상대로 돈다).
- **⑴ 의 예외 — `마감 검증: ✅ 기각 승계`.** 그 ✅ 가 아래 3) *기각* 이 남긴 승계 마커면 해소로
...
- `pr-head-at.sh` 가 exit 1(무출력)이면 "새 커밋 없음" 으로 읽지 마라 — **못 얻은 것**이다.
  해소를 증명하지 못했으니 **미해소**로 두고 2) 로 간다(fail-closed — ①-b 와 같은 방향).
- 둘 다 아니면 **미해소 보류**다. 단 **이미 시정으로 재디스패치됐으면 다시 반송하지
  않는다(멱등 — #198 반송3/P1).** 이 보류 경계 인덱스 이후에 `재디스패치: #<이슈>` 로
  시작하는 코멘트(아래 3) *시정* 행이 남기는 것과 같은 접두)가 이미 있으면,
  `stale_reverify` 행("head 커밋이 신선하면 코멘트가 낡았어도 `active` 로 떨어져 살아있는
  attempt N+1 워커를 오분류하지 않는다")이 쓰는 것과 **같은 술어를 재사용**해 "그 마커
  이후 워커 활동이 있는가" 를 잰다 — 새 헬퍼를 만들지 않고, 위에서 이미 구한
  `pr-head-at.sh` 의 head 커밋 시각을 그 마커의 `createdAt` 과 다시 비교한다. head 커밋이
  그 마커보다 **늦지 않으면**(마커 이후 새 커밋 없음) **이번 틱은 손대지 마라** — 마커도
  다시 쓰지 말고 `$SCRIPTS/transition.sh closeout-redispatch` 도 다시 부르지 마라. ①-b 의
  멱등 마커 절은 "마커가 이미 있고 그 이후 새 커밋·검증자 코멘트가 없으면 **코멘트를**
  ...
  `agent:claimed` 를 또 뗀다(검증 레인 실측 — 디스패치 대기+구현 시간 창 전체가 매 틱
  재호출 구간이었다). head 커밋이 그 마커보다 **늦으면**(새 커밋 있음) 그것은 새 워커가
  **착수**했다는 증거일 뿐 **완결**했다는 증거는 아니다 — '착수' 와 '완결' 을 한 신호(첫
  커밋)로 겸하면 절반짜리 커밋이 그대로 Pick 된다(#198 재심/P1). 이 경우도
  **이번 틱은 손대지 마라** — `active`(무접촉)로 두고, 마커도 다시 쓰지 말고
  `closeout-redispatch` 도 다시 부르지 마라. 대기를 끝내는 조건은 재디스패치로 착수한
  새 워커 자신의 **새 완결 판정**(= 이 마커 이후 posting 된 새 `머지 판정: ✅`)이 서는
  것이다 — 그 시점엔
  `finish-classify` 가 이 PR 을 독립적으로 `done_verdict` 로 분류하므로(①-b `done_verdict`
  행) ①-c 가 정상 재진입해 위 "해소 판정" 경로로 합류한다(새 판정 기준을 만들지 않는다 —
  이미 있는 `done_verdict` 자체를 완결 신호로 쓴다). 그런 마커 자체가
  없으면(이 절 첫 진입) → 2) 로 간다.

**2) 해제 결정 코멘트를 특정한다.** **그 출처 자신의** 보류 경계 인덱스 **이후**의 코멘트 중
