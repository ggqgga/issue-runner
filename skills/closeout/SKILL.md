...
| `active` | 진행 중·버퍼 미도달·우리 형상 아님, 또는 **✅ 의 신선도를 증명 못 함**(✅ 가 head 커밋보다 이르거나 두 시각 중 하나를 못 얻음, #171). **위 1) 의 CONFLICTING PR 도 `bounce-state.sh` 가 `ok` 가 아니면(반송 회차 진행 중이거나 판정 실패) 여기로 떨어진다**(#196) — 단계 라벨이 비어 있어도 그건 좌초가 아니라 워커 레인 소유다. **미해소 `마감 검증: ⚠ 보류` 가 있는 PR 은 여기서 끝내지 마라**(#198) — ①-c 가 해제 방향을 읽어 *시정*이면 `active`(무접촉)가 아니라 **반송**(`closeout-redispatch`)이고, 방향이 *모호*하면 `closeout-blocked`(사람)다. 무접촉으로 두면 다음 틱에 그대로 다시 후보가 된다 | **무접촉**(다음 틱). |
...
- 둘 다 아니면 **미해소 보류**다. 단 **이미 시정으로 재디스패치됐으면 다시 반송하지
  ...
  재호출 구간이었다). head 커밋이 그 마커보다 **늦으면**(새 커밋 있음) 새 워커가 이미
  착수한 것이므로 위 "해소 판정" ⑵ 와 같은 경로로 **해소** → ② Pick. 그런 마커 자체가
  없으면(이 절 첫 진입) → 2) 로 간다.
