...
**착수**했다는 증거일 뿐 **완결**했다는 증거가 아니다 → **`active`(무접촉)** 로 두고
**이번 틱은 손대지 마라**. 마커도 다시 쓰지 말고 `$SCRIPTS/transition.sh closeout-redispatch` 도
다시 부르지 마라. 이건 `stale_reverify` 행이 쓰는 술어와 **같은 것**이고(head 커밋이 신선하면
코멘트가 낡았어도 `active` 로 떨어져 살아있는 attempt N+1 워커를 오분류하지 않는다) 새 헬퍼를
만들지 않고 이미 있는 `pr-head-at.sh` 를 그대로 재사용한다.
**해제 경로**: 새 완결 판정이 서면 위 해소 판정이 이 칸을 푼다. 영영 안 서면 그건 정지이고
`loop-status.sh` 정지 미러 warn(#265)이 보이게 할 자리다.
...
- 이 순서의 **알려진 대가**: `(완결 없음, 새 커밋 있음, 마커 > 경계, 레인 라벨 없음)` 칸에서는
  ...
  6-b 행). 도달 조건은 좁다 — 전이가 성사되면 `agent-ready` 가 워커 수명 내내 남으므로
...
`attempt5` 열은 **직전 회차의 설계**(닫힌 PR #203 / 브랜치 `agent/issue-198`, head `683cdde2`)이고,
`뒤집힘` 열의 축④·축⑤ 가 이번 회차가 새로 닫는 칸(검증자 BLOCKER ①·②)이다.
...
| 2 | `h`, `f > max(h,r)`, `H` 없음 | ② Pick | ② Pick | ② Pick | — |
| 3 | `h`, `c` 만(`f` 없음), `r` 없음 | ② Pick | **② Pick** | **`active`** | ✅ 축① |
| 4 | `h`, `r > h`, `c`(마커 뒤 새 커밋), `f` 없음, `L` 있음 | ② Pick | **② Pick**(상류가 먼저 발화해 '착수≠완결' 에 도달 못 함) | **`active` 무접촉** | ✅ 축① |
| 5 | `h`, `r > h`, `c` 없음, `L` 있음 | ② Pick | `active` 무접촉 | `active` 무접촉 | — |
| 6 | `h`, `r > h`, `f` 없음, **`c` 없음**, **`L` 없음**(전이 실패·유실) | ② Pick | **`active` 무접촉 → 영구 정체**(워커가 영영 안 옴) | **마커 재발행 없이 전이만 재호출** · 또 실패면 `BLOCKED: 전이 실패 재디스패치` | ✅ 축② |
| 7 | `h`, `r > h`, `f` 없음, **`c` 있음**, **`L` 없음**(전이 실패 ∧ 사람 직접 커밋) | ② Pick | `active` 무접촉 | `active` 무접촉 — 착수 판정이 먼저다(순서 계약), 정체는 #265 warn 이 본다 | — |
| 8 | `h`, 마커 posting 실패(전이 미호출) | ② Pick | `BLOCKED` · 상태 불변 · 다음 틱 재시도 | 같음 | — |
| 9 | `h`, `r` 없음, `D` 있음, `H` 없음 | ② Pick | 3) 방향 판정 | 3) 방향 판정 | — |
| 10 | `h`, `r` 없음, `D` 있음, **`H` 있음**(라벨 아직) | ② Pick | **3) 방향 판정 진행**(기각이면 ② Pick) | **보류 유지 · 무접촉** | ✅ 축③ |
| 11 | `h`, `r` 없음, **`D` 없음**, `H` 없음 | ② Pick | 모호 → `closeout-blocked` | 같음(= 보류 유지) · 정체는 #265 warn 이 본다 | — |
| 12 | `h`, 결정문이 **이슈에만** | ② Pick | 있는 쪽 방향 채택 | 같음 | — |
| 13 | `h`, PR·이슈 결정문 방향이 **다름** | ② Pick | 모호 | 같음 | — |
| 14 | `h`, `pr-comments.sh` 조회 실패 | ② Pick | 모호(인용 면제) | 같음 | — |
| 15 | `h`, `f` 있으나 **`f < r`**(마커보다 이른 ✅) | ② Pick | **해소 → ② Pick** | **`active`**(`max(h,r)` 뒤여야 해소) | ✅ 축① |
| 16 | 워커가 찍은 `머지 판정: ⚠ 보류` 의 해제 | ② Pick | **경계로 안 셈 → ② Pick** | 경계로 세고 → 2) 논리곱 → 3) 방향 판정 | ✅ 축③ |
| 17 | `h`, `f` 아닌 `머지 판정: 🔄` 만 뒤에 있음 | ② Pick | **완결로 새어 해소** | **완결 아님** → `active`/멱등 분기 | ✅ 축① |
| 18 | `h`, `r > h`, `c` 없음, `f` 없음, 워커 두 라벨은 **없고** 이슈에 **`flow:verify` 있음**(`handoff-verify` 착지, 검증자 판정 전 창) | ② Pick | **전이 재호출 → 살아있는 검증 레인의 `flow:verify` 를 뗌** | **무접촉**(하류 활성 레인 소유) | ✅ 축④ |
| 19 | `h`, `pr-head-at.sh` **exit 1**(값 없음), 이슈에 `flow:verify` 만 | ② Pick | **'읽어 내려가라' → 실측 갈래 → 전이 재호출**(fail-open) | **`BLOCKED: head 조회 실패` · 상태 불변 · 무접촉** | ✅ 축④ |
| 20 | `h`, `f > max(h,r)`, **`H` 있음**(사람이 `needs-human` 만 떼고 `hold:policy` 를 남김) | ② Pick | **해소 → ② Pick**(살아 있는 사람 게이트를 벗김) | **보류 유지 · 무접촉** | ✅ 축⑤ |
