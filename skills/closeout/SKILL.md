...
- **거짓 BLOCKER 반전 기록 (lessons).** 이 PR 에 이전 틱의 `마감 검증: ⚠ 보류 — …`
  ...
  **캡: 항목 20개** — 초과 시 **항목 수가 캡 이하가 될 때까지** 가장 오래된 항목부터
  통째로 삭제한다(줄 단위가 아니다. 이 파일에는 `##` 로 시작하는 여러 줄짜리 사례가
  섞여 있어 줄을 자르면 산문이 찢어진다. 항목 = `- [` 로 시작하는 한 줄, 또는 `##`
  헤더부터 다음 항목 직전까지). 정리는 손으로 세지 말고 `scripts/lessons-trim.sh
  <파일> 20` 한 자리로 부른다(append 직후 이 한 줄이면 된다 — #208: 옛 규칙은
  "가장 오래된 항목 **하나만**" 삭제라 append(+1)·삭제(-1) 순증이 0 이라서 한 번
  캡을 넘으면 영원히 안 줄었다).
