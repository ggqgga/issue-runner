...
| `active` | 진행 중·버퍼 미도달·우리 형상 아님, 또는 **✅ 의 신선도를 증명 못 함**(✅ 가 head 커밋보다 이르거나 두 시각 중 하나를 못 얻음, #171). **미해소 `마감 검증: ⚠ 보류` 가 있는 PR 은 여기서 끝내지 마라**(#198) — ①-c 가 해제 방향을 읽어 *시정*이면 `active`(무접촉)가 아니라 **반송**(`closeout-redispatch`)이고, 방향이 *모호*하면 `closeout-blocked`(사람)다. 무접촉으로 두면 다음 틱에 그대로 다시 후보가 된다 | **무접촉**(다음 틱). |
