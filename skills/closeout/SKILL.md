...
| `held` | 최신 `머지 판정: ⚠ 보류` (워커 명시 보류 — ①-c 의 **보류 경계** 셋 중 하나다) | **해제가 이미 일어났으면**(①-c 2) 의 논리곱이 참 — 창 뒤 결정 코멘트 ∧ `needs-human`·`hold:*` 현재 부재) **①-c 로 보낸다**(시정→`closeout-redispatch` 반송, 모호→`closeout-blocked`, 기각→② Pick) — 그 전에 `closeout-blocked` 를 다시 부르면 사람이 방금 푼 보류를 **매 틱 되살린다**(#225 가 못박은 자리 — 진입만 보고 해제 경로를 안 재면 루프가 사람과 싸운다). **아직 해제 전이면 정지(`hold:policy`)** — `$SCRIPTS/transition.sh closeout-blocked <repo> <issue|-> <pr> --reason policy --note "<질문 한 줄>"` (PR 과 연결 이슈 **양쪽**에 `hold:policy` 부착 + 단계 라벨 정리 — 연결 이슈가 없어도 PR 에 정지 신호가 남는다. **`needs-human` 은 안 붙는다**(#244) — 기계 정지는 사유 라벨 하나뿐이고, 사람 호출은 재개 스윕 ③ 의 재심이 "사람 몫 유지" 로 끝났을 때만 `transition.sh policy-kept` 가 붙인다), closeout 무접촉(자동 진행 안 함). |
...
- 그런 코멘트가 **PR·이슈 양쪽 배열에** 하나도 없으면 → 보류가 없었다 → **`H`(이슈·PR 의
  ...
  이슈측 경계를 **이미 계산**하므로 비용은 조건 하나다. 이슈 쪽에만 경계가 있으면 그 경계로
  아래 판정을 이어간다(`h` 있음으로 읽는다) — 이 형상을 아래에서 **이슈측 단독 경계**라 부른다.
  **단 인덱스는 배열을 넘지 않는다(#334 attempt 2 P1)**: 이슈측 경계를 "`h` 있음" 으로 읽는 것은
  *진입* 판정(보류가 있었다)까지이고, 뒤의 **PR 배열 인덱스 비교 두 자리** — 해소 판정 ⑴ 의
  `f > max(h, r)` 와 멱등 분기의 `r > h` — 에 이슈 배열의 인덱스를 넣지 마라. 그 두 자리는
  각각 이슈측 단독 경계용 규칙을 따로 적었다(⑴: 해소 후보 없음 · 멱등: `createdAt` 엄격 비교).
...
그래서 이슈측 단독 경계 형상에서 PR 배열에 **재디스패치 마커도 없으면** 비교할 같은 배열의
기준이 없다 → **해소 후보를 내지 않는다**(보수적 — 여는 갈래는 증명 없이 열지 않는다).
그 형상은 착수·멱등 분기를 지나 2)·3) 으로 내려가 **이슈측** 결정문으로 방향만 판정한다(2) 의
결정문 특정은 원래 출처마다 따로라 배열을 안 넘는다). **해제 경로**: 시정이면 3) 이 PR 에
재디스패치 마커를 세워 다음 틱부터 PR 배열에 기준 `r` 이 생기고, 그 뒤 새 완결 판정이 서면
이 ⑴ 이 푼다(`f > r`, 같은 배열); 기각이면 3) 기각 행이 `✅ 기각 승계` 를 남기고 ② Pick 으로
간다(⑴ 을 거치지 않는 길 — 사람이 원안 머지를 명시한 것이다); 모호면 `closeout-blocked` 가
PR·이슈 **양쪽**에 `<!-- hold-note: ` 를 다시 남겨 정상 형상(양쪽 경계)으로 돌아온다.
PR 배열에 마커 `r` 이 **있으면** 그 `r` 이 같은 배열의 기준이므로 `f > r` 로 정상 판정한다.
...
**착수**했다는 증거일 뿐 **완결**했다는 증거가 아니다(이 비교는 원래 시각끼리라 배열을 안 넘는다 —
이슈측 단독 경계 형상에서 `r` 이 없으면 이슈측 경계 코멘트의 `createdAt` 을 쓴다). 다만 **누가 착수했는가**에 따라 목적지가
...
둘 다 **PR 배열** 인덱스다. **이슈측 단독 경계** 형상(PR 배열에 경계 0)에서는 같은 배열의 `h`
가 없으므로 인덱스 대신 **`createdAt`** 으로 가른다(#334 attempt 2 P1): 마커의 `createdAt` 이
이슈측 경계의 `createdAt` 보다 **엄격히 늦을 때만** `r > h` 다. **동초·이전이면 `r ≤ h`** 로
읽어 2) 로 간다(이 절이 아직 판정 안 한 새 보류로 본다 — fail-closed). 시각을 배열 밖에서
쓰는 유일한 자리이고 허용하는 근거는 비대칭이다: 여는 갈래(⑴)가 아니라 두 fail-closed 갈래
사이를 가르는 자리라, 동초 오독의 대가는 2) → 시정 → 마커 재발행 한 번이고, 반대 오독(새
보류를 옛 마커 뒤로 읽음)의 대가는 그 보류의 `hold:*` 를 아래 *전이 재호출* 이 벗기는 것이다.
...
- ⓑ 가 거짓(**라벨이 아직 있다**) → 사람이 아직 들고 있다 → **무접촉**. 전이를 다시 부르지
  ...
  라벨이 남아 있으면 사람이 아직 안 놓은 것이다. **해제 경로**: 사람이 라벨을 뗀다.
...
| **결정문 있음 + 라벨 아직** | 결정문은 달렸는데 `hold:policy` 가 이슈에 남음(`D` 참 · `H` 참) | 2)ⓑ 거짓 → **보류 유지 · 무접촉** · 전이를 다시 부르지 않는다 |
...
| **이슈측 단독 경계 + 낡은 ✅** | PR 배열엔 경계·마커 0, 이슈 배열엔 `<!-- hold-note: `(인덱스 2), PR 에 보류 **이전**의 `머지 판정: ✅`(인덱스 5), `H` 없음 | ⑴ **해소 후보 없음**(배열 교차 비교 금지) → 2)·3) 이슈측 결정문으로 방향 판정 — 격자 28행 |
...
"뒤" 로 읽혀 오머지). **축⑦ 행의 `반송 전` 열은 이 PR 의 직전 커밋
(`d82c6e7`)**, **축⑧ 행의 그 열은 attempt 1 커밋(`fbdcfba`)** 의 동작이고, 축①~⑥ 행의 그 열은
닫힌 PR #203(head `683cdde2`)의 **10:11 반송 시점** 상태다 — 세 기준이 섞여 있으므로 행마다
괄호로 밝힌다. `want` 열만이 현재 계약이다.
...
| 28 | **이슈측 단독 경계**(PR 배열 경계·마커 0, 이슈 배열 `<!-- hold-note: ` 인덱스 2) ∧ PR 에 보류 **이전**의 `f`(`머지 판정: ✅`, 인덱스 5) ∧ `H` 없음 ∧ `D` 있음 | ② Pick | **`fbdcfba`: ⑴ 이 PR 인덱스 5 > 이슈 인덱스 2 로 읽어 해소 → ② Pick**(보류 이전 코드 오머지) | **⑴ 해소 후보 없음**(같은 배열의 기준이 없다) → 2)·3) 이슈측 결정문으로 방향 판정 | ✅ 축⑧ |
| 29 | **이슈측 단독 경계** ∧ PR 에 `r`(앞 틱의 시정 반송) ∧ 이슈측 경계의 `createdAt` 이 `r` 보다 **늦거나 동초**(새 보류) ∧ `H` 있음 | ② Pick | **`fbdcfba`: PR 배열에 `h` 가 없어 `r > h` 미정의** — 마커 존재로 멱등 분기에 들어가면 `A` 거짓 ∧ `R` 거짓 → 전이 재호출이 새 보류의 `hold:*` 를 벗김 | **`r ≤ h` 로 읽어 2)** → ⓑ 거짓(라벨 있음) → **보류 유지 · 무접촉** | ✅ 축⑧ |
