...
|---|---|---|
| `done_verdict` | 최신 `머지 판정: ✅` **이고 그 판정이 현재 head 커밋 이후임이 증명됨**(#171) | eligible.sh 정상 경로가 처리 — 스윕은 skip. 단 그 정상 경로도 **①-c 방향 판정을 먼저 통과**해야 ② Pick 이 집는다(#198 — 사람이 "고쳐라" 라고 푼 보류는 ✅ 가 신선해 보여도 머지 후보가 아니다) |
...
(`--paginate` — 조회 로직 두 벌 금지·첫 100건 상한 회피, ①-b 와 같은 이유). 조회가 exit 1 이면
**증명 실패**이므로 통과시키지 말고 아래 3) 의 *모호* 로 간다.
...
- **보류 경계** = `마감 검증: ⚠ 보류` 로 시작하거나 `<!-- hold-note: ` 를 담은 코멘트의
  **마지막 매칭 인덱스**. `createdAt` 이 아니라 **코멘트 배열 인덱스**로 잰다 — GitHub 코멘트
  ...
  안전망이 쓰는 관용구 그대로).
- 그런 코멘트가 하나도 없으면 → 보류가 없었다 → 그대로 ② Pick.
- **해소 판정**: 그 인덱스 **뒤에** ⑴ `마감 검증: ✅` 로 시작하는 코멘트가 있거나
...
**2) 해제 결정 코멘트를 특정한다.** 보류 경계 인덱스 **이후**의 코멘트 중 아래 둘 중 하나면
결정 코멘트 후보이고, 그중 **마지막 매칭 인덱스** 하나를 취한다:
...
| 방향 | 읽는 신호 | 조치 |
|---|---|---|
| **시정** | "판정이 맞다"·"좁힌다/고친다/바꾼다"·구현 지침·추가 테스트 요구 등 **코드를 바꾸라는 문장이 하나라도** 있다 | `$SCRIPTS/transition.sh closeout-redispatch <repo> <issue> <pr>` + PR 에 `gh pr comment <pr> --repo <repo> --body "재디스패치: #<이슈> — 사람 재심이 시정 방향(<결정문 인용 한 줄>)`⏎`<!-- bodat:worker -->"`. 이 마커가 `BOUNCE_MARKERS` 에 있어 이후 틱의 `closeout-eligible.sh` 가 자동 배제한다. **② Pick 하지 않는다.** |
...
| **모호** | 결론 없이 질문만·조건부·양쪽이 섞임·결정 코멘트 부재·코멘트 조회 실패 | `$SCRIPTS/transition.sh closeout-blocked <repo> <issue|-> <pr> --reason policy --note "<해제 코멘트의 결론이 기각인지 시정인지 한 줄로>"` 로 사람에게 되돌린다(fail-closed — 증명 안 되면 열지 않는다). **② Pick 하지 않는다.** |
...
방향 판정 건수는 ④ Report 에 `방향 판정 N (기각 n · 시정 m · 모호 k)` 로 집계한다.
