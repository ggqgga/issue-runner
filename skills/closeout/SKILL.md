...
| `held` | 최신 `머지 판정: ⚠ 보류` (워커 명시 보류 — ①-c 의 **보류 경계** 셋 중 하나다) | **해제가 이미 일어났으면**(①-c 2) 의 논리곱이 참 — 창 뒤 결정 코멘트 ∧ `needs-human`·`hold:*` 현재 부재) **①-c 로 보낸다**(시정→`closeout-redispatch` 반송, 모호→`closeout-blocked`, 기각→② Pick) — 그 전에 `closeout-blocked` 를 다시 부르면 사람이 방금 푼 보류를 **매 틱 되살린다**(#225 가 못박은 자리 — 진입만 보고 해제 경로를 안 재면 루프가 사람과 싸운다). **아직 해제 전이면 needs-human** — `$SCRIPTS/transition.sh closeout-blocked <repo> <issue|-> <pr> --reason policy --note "<질문 한 줄>"` (PR 과 연결 이슈 **양쪽**에 `needs-human` + `hold:policy` 부착 + 단계 라벨 정리 — 연결 이슈가 없어도 PR 에 사람 신호가 남는다), closeout 무접촉(자동 진행 안 함). |
...
한 번이 정상 후보에 `needs-human`+`hold:policy` 를 박고 그 게이트는 사람이 **지어낼 수 없는**
...
- 그런 코멘트가 **PR·이슈 양쪽 배열에** 하나도 없으면 → 보류가 없었다 → **`H`(이슈·PR 의
  ...
  없다: ①-b 스윕 대상 필터는 `needs-human` 만 보고 `hold:*` 만 붙은 입양 후보는 안 거르며,
  `closeout-eligible.sh` 의 배제도 `done_verdict` 행이 **우회**한다. **해제 경로**: 사람이
  라벨을 뗀다.
