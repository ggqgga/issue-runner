...
- **실장비 항목이 남아 있으면 green 이어도 닫지 않는다 (칸 ③ 은 크롬이 못 밟는다).**
  ...
  **전환기 — 재고 티켓은 고정 문자열로 잡고, 그 자리에서 표식을 보강한다.** 이 규율보다
  먼저 발행돼 아직 열려 있는 배포 이슈엔 `[칸 ③]` 이 아예 없다(#309 머지 이전 발행분).
  그런 티켓에 한해 줄에 고정 문자열 `TEST 워커 프로필 #18` 이 적혀 있으면 실장비로 본다 —
  **해석이 아니라 두 번째 고정 문자열**이니 뜻풀이로 넓히지 마라. 그렇게 잡은 줄은 배포
  이슈 본문을 그 자리에서 `- [ ] [칸 ③] <동작>` 으로 고쳐 표식을 보강하고(다음 틱은 표식만
  보면 된다), 보강 사실을 코멘트에 한 줄로 남긴다. 재고가 다 소진되면 이 조항은 아무 줄도
  잡지 못하고 자연히 죽는다.
  **그 밖에 표식이 없는 줄은 크롬 대상이다** — 해석으로 실장비 승격하지 마라(그 순간
  실장비를 세는 계산기가 둘이 된다). 다만 크롬으로 도저히 대조할 수 없으면 pass 로 찍지
  말고 **fail 로 두어** 아래 fail 갈래(후속 이슈 발행)로 보낸다 — 거짓 초록보다 후속
  이슈가 낫다. 4단계 산출에서 표식이 빠진 것으로 보이면 그 사실도 코멘트에 한 줄로 남긴다.
