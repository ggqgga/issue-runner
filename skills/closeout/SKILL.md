...
3. **보류 해소 대조 (#174)** — 최신 ✅ 를 그보다 **뒤에 달린** `마감 검증: ⚠ 보류` 가 가리면,
   사람이 그 뒤에 `needs-human`·`hold:*` 를 뗐음을 `pr-hold-released-at.sh`(타임라인
   `unlabeled` 이벤트, `--paginate` 전량)로 **증명**했을 때만 ✅ 가 살아난다. 조회 실패·해제
   이벤트 없음·동초는 전부 `active`(무접촉)다 — `held` 가 아닌 이유는 `held` 가 스윕에
   `closeout-blocked`(= `needs-human` **부착**)를 시켜 사람이 방금 뗀 보류를 루프가 다시
   붙이는 경로가 되기 때문이다(#151). 보류가 최신 ✅ 보다 **앞**이면 이미 지나간 보류라
   아무것도 가리지 않는다(반송 후 재완결이 영구 억제되지 않게). 해제 **이전**의 사람
   코멘트는 그 해제로 답해진 것이므로 "미해결 사람 코멘트" 로도 세지 않는다 — 운영자
   결정문이 큐를 막던 실측 경로다.
...
**뒤에** 사람이 `needs-human`·`hold:*` 를 뗐다면 — 그래서 이 PR 이 다시 후보로 올라왔다
