...
- `note` — **아무것도 안 건드린** 정상 상태(`Epic #N` 줄이 없는 옛 에픽 · `deploy-wait`
  ...
  이슈 #M 열림`, #343 — 프로덕션에 안 올라간 에픽은 끝난 게 아니다). **보고하지 않는다** —
  매 틱 같은 줄이 쌓여 진짜 신호를 묻는다.
- `warn` — 판정을 **보류**했거나(leaf·배포 대기 검색이 상한에 닿음) 조회·쓰기가 실패했다.
  ④ Report 의 warn 줄에 `why` 를 그대로 옮긴다. 보류는 실패가 아니므로 exit 0 일 수 있다.
