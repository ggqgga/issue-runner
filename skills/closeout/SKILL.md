...
**분기 — 밟을 항목이 없으면 이슈를 만들지 않는다.**

- **`없음` 인 경우: 이슈를 발행하지 않는다.** `gh pr comment <pr> --repo <repo> --body
  "배포 대기(검증 항목 없음) — 승격 시 함께 올라간다`\n`<!-- bodat:worker -->"` 마커만
  남기고 → **approval-required 로 종료**한다. **5단계 스모크도 건너뛴다** (밟을 항목이
  0인데 Chrome 을 띄울 이유가 없다 — 실측: 항목 `없음` 64건 중 39건이 스모크를 돌렸다).
  - 이게 유실이 아닌 이유: **배포 대기 이슈는 "무엇이 미승격인가"의 진실 원본이 아니다.**
    그 답은 ④ Report 의 `승격 대기 N커밋` 과 승격 측의 `git merge-base --is-ancestor` +
    태그가 이미 갖고 있다. 이슈는 **사람이 밟을 항목의 그릇**이지 배포 원장이 아니다.
  - 승격 담당(사람)이 보는 목록에서 이 PR 이 빠지는 것도 의도다 — 밟을 게 없으니 볼 게 없다.
- **체크박스가 하나라도 있는 경우:** 기존대로 `gh issue create --repo <repo> --label
  needs-human` 으로 배포 요청 이슈를 발행하고, `gh pr comment <pr> --repo <repo> --body
  "배포 대기: #<생성번호>"` 로 마커를 남긴 뒤 → **approval-required 로 종료**한다.
  체크박스 목록이 그대로 그 이슈가 닫히는 조건이 된다.

**묶지 않는다.** 여러 배포 대기 이슈를 하나로 합치지 마라 — 항목이 계속 붙는 장수 이슈는
닫히는 시점이 사라져 "끝나지 않는 이슈" 가 된다(사용자 결정, 2026-08-13). 줄이는 축은
**개수를 합치는 것이 아니라 밟을 게 없는 건을 애초에 안 만드는 것**이다.
...
- **밟을 항목이 없으면 스모크하지 않는다.** 4단계가 `없음` 분기로 이슈를 안 만든 PR 은
  5단계 대상이 아니다(스모크할 이슈 자체가 없다). 배포 이슈가 있더라도 `## 라이브/하드웨어
  검증 항목` 에 `- [ ]` 가 하나도 없으면 Chrome 을 띄우지 말고 완료로 넘긴다 — 대조할
  항목이 0인 스모크는 무엇을 통과시킨 게 아니라 **아무것도 안 본 것**인데 `✅ 스모크
  0/0 통과` 로 찍히면 검증된 것처럼 읽힌다(거짓 초록). 이때는 그 사유를 코멘트에 남긴다:
  `스모크 생략: 밟을 항목 0`.
