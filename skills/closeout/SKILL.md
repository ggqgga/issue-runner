...
- **표면 교정 흡수 (같은 커밋에 얹는다).** 1단계 검증자의 WARN/NIT 중 **표면 교정**
  ...
  **판정 기준 한 줄: 이 변경으로 통과/실패가 바뀌는 테스트가 하나도 없는가.**
  ...
  설명 문자열은 실행되지만 통과/실패를 안 바꾼다), **이 PR 이 도입한 가드·테스트의 토큰
  보강·앵커 동기·이름 정정**(무는 동작이 그대로인 것 — 2026-09-13 실측: bin/ci 가드 토큰 추가·정규식
  끝 앵커 동기·주석 정정이 이슈 4건으로 한 바퀴씩 돌았다, #411). 이 기준이 막는 것 — 다른 동작을
  새로 무는 단언·가드, 커버리지 추가·상수값·실행 분기. "주석만 고치는 김에 단언 하나" 는 이슈다.
...
**6단계 — 파생 이슈.** 입력은 워커 PR 본문의 `follow-up:` 항목 + 1단계 diff 리뷰가 짚은
인접 작업이다. **입력을 이슈로 옮겨 적지 않는다 — 항목마다 먼저 판정하고, 이슈는 다섯 갈래 중
마지막 ⓔ 뿐이다**(사용자 결정 2026-09-13, #411). 리뷰어는 입력이지 결정권자가 아니다 —
"codex 가 P2 로 냈으니 이슈" 는 판정이 아니다. 실측: 이 레포의 WARN 잔여 파생 10건을 사람과
다시 읽으니 이슈가 맞는 것은 0건이었다(가드 토큰·앵커·주석 4 · 가설 시나리오 2 · 형제 PR 이
이미 갈아엎은 2 · 소음 1 · 결정 요청 1).

| 항목의 성격 | 처리 |
|---|---|
| ⓐ 이 PR 파일 안에서 끝나고 동작이 안 바뀐다(주석·용어·앵커·가드 토큰·테스트 이름) | **흡수** — 3단계 표면 교정 커밋이 먹었어야 할 부류. 머지 뒤라 커밋 자리가 없으면 이슈가 아니라 아래 판정 코멘트에 `흡수 누락:` 으로 적는다 |
| ⓑ 루프 정상 동작 밖 시나리오(사람의 히스토리 재작성 · 문서 밖 설정값 · 상한 초과 설정) | **기각** — 판정 코멘트에 `기각: <사유>` |
| ⓒ 대상 코드를 형제 PR 이 바꾸는 중이거나, 발행 시점에 `origin/<default>` 에 그 줄이 없다(`git grep` 으로 확인) | 이미 머지돼 사라졌으면 **기각**. 형제 PR 이 열려 있으면 **지적을 그 PR 로 전달**한다 — 형제 PR 과 그 연결 이슈에 `인접 지적(closeout 6단계, PR #<원본>): <무엇을 왜> <!-- bodat:worker -->` 코멘트를 남기고 판정 코멘트에 형제 PR 번호를 적는다(마커가 없으면 그 코멘트가 형제 PR 을 미해결 사람 코멘트로 붙든다, #379). 다른 레인의 PR 을 closeout 이 반송하지는 않는다(소유권) — 형제가 검증 전(`flow:claimed`·`flow:verify`·`verifying`)이면 검증자가 그 코멘트와 diff 를 함께 보고, 이미 ✅ 뒤(`flow:ready`·`harvesting`)면 아무도 읽지 않으니 **ⓔ 로 판정**한다 |
| ⓓ 사람이 정해야 하는 갈림길("둘 중 하나로") | **needs-human 코멘트** 한 줄 — 부모 에픽이 있으면 에픽에, 없으면 이 PR 에. 이슈를 만들지 않는다 |
| ⓔ ⓐ~ⓓ 어디에도 들지 않고 남는, 고쳐야 하는 **실제 결함** — 이 PR 밖 코드든, 3단계가 "동작이 바뀐다" 며 넘긴 이 PR 안의 것이든. 출처(검증자 P2 · 범위 밖이라 WARN 으로 낮춘 P1 · 워커 `follow-up:` 항목)는 판정 조건이 아니다 — follow-up 항목도 ⓐ~ⓓ 를 먼저 거친다 | **이슈** — 아래 발행 명령으로 `references/spinoff-issue.md` 를 채워 agent-ready 이슈로 발행. 본문 둘째 줄 `Spinoff of PR #<pr> (issue #<부모>)` 필수 |
...
- **발행 직후 확인.** `gh issue view <번호> --repo <repo> --json labels,body` 로
  ...
  ⑶ **출처 줄도 같은 자리에서 확인한다** — 본문에 `Spinoff of PR #<pr> (issue #<부모>)` 전용 줄이
  정확히 하나 있어야 한다(템플릿 `<ORIGIN_LINE>` 자리, #411). 없으면 ⑵ 와 같은 방법으로 즉시 고친다 —
  이 줄이 없는 파생은 어디서 왔는지 사람이 산문을 뒤져야 한다(2026-09-13 실측: 최근 30건 중 12건만 적혀 있었다).
