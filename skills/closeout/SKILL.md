...
- 그런 코멘트가 **PR·이슈 양쪽 배열에** 하나도 없으면 → 보류가 없었다 → **`H`(이슈·PR 의
  ...
  아래 판정을 이어간다(`h` 있음으로 읽는다). **`H` 가 있으면 무접촉(보류 유지)** — 사람이
...
**착수**했다는 증거일 뿐 **완결**했다는 증거가 아니다. 다만 **누가 착수했는가**에 따라 목적지가
...
박음 · 경계 0 판정을 PR 배열로만 함). **축⑦ 행의 `반송 전` 열은 이 PR 의 직전 커밋
(`d82c6e7`)** 의 동작이고, 축①~⑥ 행의 그 열은 닫힌 PR #203(head `683cdde2`)의 **10:11 반송
시점** 상태다 — 두 기준이 섞여 있으므로 행마다 괄호로 밝힌다. `want` 열만이 현재 계약이다.
