...
## 완료 처리
위 체크박스를 전부 밟으면 이 이슈를 닫는다. 실장비 항목(접두 표식 `[칸 ③]` — 사다리 칸 ③, TEST 워커 프로필 #18 드라이런)이 `- [ ]` 로 하나라도 남아 있으면 closeout 5단계는 닫지 않는다 — 그 칸은 deploy-cycle ⑦ 이 밟고 닫는다. 문제가 있으면 closeout 5단계가 후속 이슈를 발행한다.
