...
**범위 밖으로 남겨두는 것 — 바인드 직후 소유권 미확인 시 비-multiplex 로 이미 뜬 포워딩
프로세스 자체.** 시작-전 잔재 체크로 창이 좁아지긴 했지만, 그 체크와 실제 `ssh -M` 사이의
아주 좁은 경합(TOCTOU)에서는 이론상 여전히 비-multiplex 폴백이 발생할 수 있다. 이 경우
`-O check` 실패로 `CTL` 을 채택하지 않아 **남의 master 를 끊는 사고**와 **거짓 성공 보고**는
막지만, 그 순간 이미 백그라운드로 뜬 로컬 포워딩 프로세스 자체는 control socket 이 없어
`-O exit` 으로 못 끄고 `pkill` 은 금지(위 참고)라 끌 방법이 없다 — 빈도가 극히 낮고(크래시
잔재+경합 창이 겹쳐야 함) 영향도 로컬 포트 하나 점유에 그친다. 이 이슈는 **소켓 소유권**만
다룬다(위 「걸러선 안 되는 것」) — PID 추적으로 이 잔여 프로세스까지 끄는 건 범위 밖으로
남긴다.
