...
안 열리는 것**이다(2026-09-10 BoDAT 실측: 미니의 LAN 주소·mDNS 이름은 크롬에서만 막히고
Tailscale 주소·루프백은 정상. DNS 도 권한도 아니다). 이때는 `스모크 skip` 이 아니라
같은 박스의 다른 주소로 한 번 더 시도한다 — ① 테일넷 주소 ② SSH 터널
(`ssh -f -N -L 3100:127.0.0.1:3000 <호스트별칭>` → `http://127.0.0.1:3100`, 끝나면
`pkill -f "L 3100:127.0.0.1:3000"`). 둘 다 실패했을 때만 도달 불가로 판정한다.
