...
⚠️ **경로에는 이 호출의 고유 식별자(PID)를 섞는다 — `/tmp/smoke-tun-<포트>-<pid>.sock`
(`$$`).** 포트만으로 경로를 정하면(`/tmp/smoke-tun-<포트>.sock`) 그 경로는 **예측 가능**해서
...
난다. 그래서 소유권은 검사가 아니라 **경로**로 세운다. `$$` 를 섞으면 그 경로는 이 셸
호출의 것이라 다른 호출이 물 수 없고, 덤으로 두 가지가 함께 사라진다:
...
for _try in 1 2 3 4 5; do
  P=$(( 39000 + RANDOM % 1000 ))
  C="/tmp/smoke-tun-$P-$$.sock"       # 포트 + 이 호출의 PID = 이 호출만의 경로(남이 못 문다)
  if [ -e "$C" ]; then
    # 고유 경로라 사실상 안 나오는 갈래(PID 재사용 + 같은 포트 + 앞 회차 잔재가 겹쳐야
    # 한다). 나더라도 우리가 만든 것이라는 보장은 없으니 지우지 말고 포트만 바꾼다.
    continue
  fi
  if ssh -f -N -M -S "$C" -o ExitOnForwardFailure=yes \
       -L "127.0.0.1:$P:127.0.0.1:<원격포트>" <호스트별칭> 2>"$ERR"; then
    # 경로가 이 호출 것이므로 여기 잡히는 master 는 남의 것일 수 없다. 그래도 살아 있는지는
    # 확인한다 — 고유 경로에서 -O check 실패는 "남의 master" 가 아니라 "우리 쪽 이상" 이다.
...
if [ -z "$PORT" ] && [ "$ALL_BIND" = 1 ]; then
  echo "tunnel port exhausted"        # 5회 전부 바인드 충돌(또는 master 미확인) — 로컬 문제, 도달 불가 아님
elif [ -z "$PORT" ]; then
...
자체.** 고유 경로에서는 남의 master 를 물 일이 없으니 이 갈래에 오는 건 우리 쪽 이상(예:
...
정작 확인하려던 화면을 못 본다(확인한 건 `/up` 뿐). `CTL` 은 이 호출의 PID 가 박힌
경로이므로 정리 시점에 `-O exit` 을 걸어도 남의 master 를 끊을 위험은 없다 — 다만 스모크가
오래 걸려 그 사이 master 가 죽었을 수는 있으니(정상 종료·크래시 불문) 아래 `-S` 존재
확인은 그대로 남겨 둔다. 정리는 스모크가 끝난 뒤,
**통과·실패·중단 어느 경로에서든** 한 번 — 위 출력에 찍힌 **경로를 문자 그대로** 옮겨
적는다(포트에서 역산하지 않는다. 이 스니펫의 `$$` 는 터널을 연 호출의 PID 와 다르다):

```bash
CTL="<터널 스니펫 출력의 control socket 경로>"   # 예: "tunnel ok on 39441, control socket /tmp/smoke-tun-39441-51234.sock" → /tmp/smoke-tun-39441-51234.sock
if [ -S "$CTL" ]; then
  ssh -S "$CTL" -O exit <호스트별칭> 2>/dev/null || echo "control socket 정리 실패: $CTL"
else
