...
**실호출 스모크(선택, `bin/ci` 밖).** `scripts/codex-review-gate-smoke.sh` 는 발견이 확실히 나는 diff 를 담은 임시 레포에 게이트를 **실제로** 1회 호출해 verdict 가 `NONE` 이 아님을 단언한다. `scripts/tests/` 의 스텁 격자는 이 부류의 결함을 구조적으로 못 본다 — 스텁은 발견 항목·이벤트 기록을 시키는 대로 내주므로, 진짜 CLI 가 그것을 **실제로 렌더하는지**는 스텁 밖의 사실이다. 네트워크와 codex 로그인이 필요해 CI 게이트에는 넣지 않는다; 게이트 판정부·프롬프트 계약문을 건드린 뒤, codex CLI 업그레이드 뒤, 계획부합 판정이 폴백으로만 날 때 손으로 돌린다. exit 0 = 통과, 1 = 회귀, 2 = 못 돌림(codex 부재), 3 = 미결정(발견이 안 렌더됨 — 무죄의 증거가 아니니 다시 돌려라).
