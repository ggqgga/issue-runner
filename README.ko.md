...
세 루프가 라벨을 어긋나게 옮기지 않도록, 라벨 이동은 전부 `scripts/transition.sh` 한 곳을 지난다 — `transition.sh <전이> <owner/repo> <이슈|-> <PR|->` 가 PR 과 연결 이슈를 한 호출로 옮기고(전이 표는 그 파일 상단이 SSOT), 편집 뒤 라벨을 다시 읽어 어긋나면 exit 1·gh 실패면 exit 2 로 실패를 드러낸다. 그리고 "지금 무엇이 걸려 있는가"는 `scripts/loop-status.sh` 가 찍는다 — 라벨·PR 상태만 읽는 순수 읽기 스냅샷(쓰기 0)으로, 레포별로 배포대기·사람대기·마감중·마감대기·검증대기·구현중·대기 버킷과 창 안의 실패·파생·승격 대기, 그리고 불변식 warn(무소속 PR·단계 라벨 중복·미러 불일치·좌초형·needs-human 사유 없음)을 낸다. 사람대기 줄에는 사유 라벨(`hold:conflict`·`hold:policy`·`hold:ladder`)이 병기되고, 질문 한 줄이 필수인 `hold:policy`·`hold:conflict` 인데 **같은 사유의** `<!-- hold-note: <사유> -->` 코멘트가 없으면 `질문 없음` 이 함께 찍힌다(마커는 홀드가 풀려도 남으므로 낡은 사유의 마커는 질문으로 세지 않는다)(사람이 무엇을 답해야 할지 없는 홀드) — 유무를 못 본 건은 `질문 없음` 으로 접지 않고 warn `질문 유무 미확인` 으로 낸다(조회 상한은 `HOLD_NOTE_MAX`, 기본 50). `closeout-dup` 으로 닫힌 PR 은 `중복종료` 로 따로 센다. `hold:ladder` 로 멈춘 이슈는 `scripts/resume-sweep.sh`(issue-runner ① Reconcile)가 `RESUME_AFTER_MIN` 뒤 자동 재개한다(상한 `LADDER_RESUME_LIMIT` 초과 시 `hold:policy` 로 승격) — 실측 필요는 멈출 이유가 아니라 다음 할 일이며, 통로는 `references/live-verification-ladder.md` 가 정한다. 세 루프가 ④ Report 끝에 이 출력을 그대로 붙이고, 사람은 손으로 친다 — 루프 세션 cwd(`.loop/repos` 가 있는 곳)에서는 인자 없이 `scripts/loop-status.sh`, 아니면 `scripts/loop-status.sh --repo owner/repo`.
