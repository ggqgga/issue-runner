...
세 루프가 라벨을 어긋나게 옮기지 않도록, 라벨 이동은 전부 `scripts/transition.sh` 한 곳을 지난다 — `transition.sh <전이> <owner/repo> <이슈|-> <PR|->` 가 PR 과 연결 이슈를 한 호출로 옮기고(전이 표는 그 파일 상단이 SSOT), 편집 뒤 라벨을 다시 읽어 어긋나면 exit 1·gh 실패면 exit 2 로 실패를 드러낸다. 그리고 "지금 무엇이 걸려 있는가"는 `scripts/loop-status.sh` 가 찍는다 — 라벨·PR 상태만 읽는 순수 읽기 스냅샷(쓰기 0)으로, 레포별로 **"누가 들고 있나"** 로 이름 지은 10줄 버킷(#276·#577) — 대기·**반송대기**·issue-runner·검증대기·verify-runner·마감대기·closeout·**보류**·needs-human·배포대기(`X대기` 는 그 루프가 집기 전, 루프 이름은 그 루프가 지금 들고 있음; 라벨 대응은 `agent-ready` 만 → `agent-ready`+`verify:반송` → `agent:claimed` → `flow:verify` → `verifying` → `flow:ready` → `harvesting` → `hold:*` → `needs-human`/`hold:conflict`+`full-cycle` → `deploy-wait`, 표는 `scripts/loop-status.sh` 머리 주석이 SSOT) — 과 창 안의 실패·파생·승격 대기, 그리고 불변식 warn(무소속 PR·단계 라벨 중복·미러 불일치·**정지 미러 불일치**·좌초형·블로커 needs-human·**워커 사망 의심**)을 낸다. PR 은 이슈 칸을 **6쌍**으로 미러한다(#281·#282 — 이슈 `agent-ready` 만 ↔ PR `flow:agent-ready`, `agent:claimed` ↔ `flow:claimed`, 그 뒤 `flow:verify`·`verifying`·`flow:ready`·`harvesting` 은 같은 이름): 대기 줄과 issue-runner 줄도 검증대기 줄처럼 연결 PR 을 `#i ← PR #n` 으로 그리고(이슈 칸과 일치할 때만), 어긋나면 양방향으로 warn `미러 불일치 — 이슈 flow:verify · PR flow:claimed` 가 뜬다(워커 칸 두 쌍은 PR 이 그 라벨을 달고 이슈가 정지 중이 아닐 때만 대조 — 홀드 전이는 그 둘을 일부러 안 맞춘다; 이슈 대기 칸 + PR `flow:claimed` 는 홀드 해제·claim 회수 뒤의 **재claim 대기** 모양이라 warn 대신 대기 줄에 붙는다). `무소속 PR` warn 은 단계 라벨이 **정말 하나도 없는** 열린 agent PR 만이다 — 라벨 0 PR(옛 워커·사람이 연 PR)에는 종전 인계 전 창(`HANDOFF_GRACE_MIN`, 기본 90분) 규칙이 폴백으로 남아 창 안이면 issue-runner 줄에 `← PR #n(인계 전)`, 밖이면 warn + 사망 의심 꼬리표다. `flow:claimed` PR 은 창과 무관하게 issue-runner 줄에 붙고, 창 밖인데 **claim 경과**(가장 최근 `agent:claimed` 시각 기준)도 창 이상이면 warn `워커 사망 의심 #i ← PR #n — agent:claimed 인데 N분 경과` 가 따로 뜬다(재디스패치 건은 claim 이 최근이라 안 뜬다). `반송대기` 도 `대기` 의 갈래다(#577) — 검증에서 반송된 이슈(`verify-redispatch` 가 이슈에 붙이는 `verify:반송`)만 따로 센다. 그 표식이 없던 시절엔 반송된 이슈와 한 번도 안 집힌 새 이슈가 목록에서 완전히 같아 보였다(둘 다 `agent-ready` 만 남는다). 게이트는 이 라벨을 읽지 않아 반송 건은 계속 후보로 나오고, `claim-issue.sh` 가 집는 그 edit 에서 뗀다(안전망은 `handoff-verify`·`release-labels.sh`). `막힘` 은 `대기` 의 갈래다 — `대기` 판정을 통과한 이슈 중 열린 블로커(본문 줄 시작의 `Blocked by #N` ∪ 라벨 `blocked-by:<N>` — 규칙의 SSOT 는 `eligible-issues.sh`)가 있는 건이 `#4986 ← #4985(needs-human)` 처럼 블로커와 그 버킷을 달고 내려간다(대기·반송대기에서 옮겨 온 것이라 `열림 N` 은 그대로 — 못 집는 쪽이 이긴다). 블로커 상태는 이미 받은 열린 이슈·열린 PR 목록 안에서만 판정해 **추가 gh 호출이 0** 이고, 블로커가 needs-human·배포대기(사람이 답해야 풀리는 게이트)면 블로커 기준으로 묶어 warn `블로커 needs-human #4985(bodat) — 하위 #4986 #4930 정체` 한 줄이 뜬다. `보류` 는 기계 정지 칸이다(#244) — `hold:*` 가 있는데 `needs-human` 이 없는 이슈(사다리 재개 대기 · `hold:policy` 재심 전 · 단독 `hold:conflict` 의 충돌 재개 대기, #345)가 사유를 달고 온다(`#4772(ladder)`). 단독 `hold:conflict` 는 사유 뒤에 **재개 횟수/상한**이 붙는다(`#43(conflict, 0/1)` — 횟수는 `<!-- conflict-resume: N -->` 마커 코멘트 수, 상한은 `CONFLICT_RESUME_LIMIT`; 조회는 needs-human 의 질문 조회와 같은 경로·같은 상한 `HOLD_NOTE_MAX` 를 쓰되 보류 conflict 이슈당 코멘트 조회 1회가 더 든다; 못 셌으면 횟수를 빼고 warn `재개 횟수 미확인`, #346). 우선순위는 `배포대기 > needs-human > 보류 > 단계 라벨(뒤 단계가 이김) > 막힘 > 대기` 로 단계 라벨보다 앞이고, 표시 순서는 그 역순(사다리 → 보류 → needs-human → 배포대기)이다 — 기계가 멈춘 건은 그 단계를 들고 있는 루프가 없기 때문이고, 이 칸이 없으면 그 이슈들이 `대기`(집을 수 있는 이슈) 로 읽힌다. needs-human 줄(`needs-human`, 그리고 사람이 `full-cycle` 로 인수한 `hold:conflict`)에는 사유 라벨(`hold:conflict`·`hold:policy`·`hold:ladder`)이 병기되고, 질문 한 줄이 필수인 `hold:policy`·`hold:conflict` 인데 **같은 사유의** `<!-- hold-note: <사유> -->` 코멘트가 없으면 `질문 없음` 이 함께 찍힌다(사람이 무엇을 답해야 할지 없는 홀드 — 마커는 홀드가 풀려도 남으므로 낡은 사유의 마커는 질문으로 세지 않는다) — 유무를 못 본 건은 `질문 없음` 으로 접지 않고 warn `질문 유무 미확인` 으로 낸다(조회 상한은 `HOLD_NOTE_MAX`, 기본 50). `closeout-dup` 으로 닫힌 PR 은 `중복종료` 로 따로 센다. `hold:ladder`·단독 `hold:conflict` 로 멈춘 이슈는 `scripts/resume-sweep.sh`(issue-runner ① Reconcile)가 `RESUME_AFTER_MIN` 뒤 자동 재개한다(`ladder` 는 `LADDER_RESUME_LIMIT` 회, `conflict` 는 워커 한 회차 더 — `CONFLICT_RESUME_LIMIT` 회, #345; 상한 초과 시 둘 다 `hold:policy` 로 승격하고 `policy` 는 재심 1회 뒤 사람 몫) — 실측 필요는 멈출 이유가 아니라 다음 할 일이며, 통로는 `references/live-verification-ladder.md` 가 정한다. 그래서 대시보드의 `보류` 는 루프가 풀 칸이고 블로커 warn(`블로커 needs-human …`)의 대상이 아니다. `승격 대기` 줄 위에는 `에픽` 절(#260)이 있다 — 열린 `epic` 라벨 이슈마다 leaf(본문의 `Epic #N` 전용 줄 — 줄 끝까지 그 줄뿐이어야 한다) 의 종료/전체·현재 버킷 분포·P 분포를 한 줄로 찍고, leaf 가 하나 이상인데 전부 닫혔으면 `에픽 leaf 전부 종료`, 열린 leaf 의 P 가 둘 이상 섞이면 `에픽 내 P 혼재` warn 을 낸다. 세 루프가 ④ Report 끝에 이 출력을 그대로 붙이고, 사람은 손으로 친다 — 루프 세션 cwd(`.loop/repos` 가 있는 곳)에서는 인자 없이 `scripts/loop-status.sh`, 아니면 `scripts/loop-status.sh --repo owner/repo`. 세 루프는 `--post <루프> --delta "<한 줄>"` 로 레포마다 고정 이슈 **루프 현황**(라벨 `loop-dashboard`, 첫 줄 마커 필수)의 본문을 매 틱 덮어쓴다 — 깃헙에서 그 이슈 하나만 열면 누가 들고 있고 각 루프가 마지막으로 언제 돌았는지 보인다.
...
| `MAX_OPEN_PRS` | `14` | **레포별** 열린 PR 배압. 캡에 닿은 레포의 신규 디스패치만 멈추고(보수·다른 레포는 계속) Report 에 캡 도달 레포당 적체 warn 한 줄 (#362) |
| `MAX_REPAIRS_PER_PR` | `3` | PR 당 보수 상한. 초과하면 멈추고 이슈에 `needs-human` 라벨 (서킷 브레이커) |
| `ISSUE_TIMEBOX_HOURS` | `1` | PR 없이 이만큼 지나면 **진행 증거**를 묻는다 — 경과 초과 자체는 더 이상 중단 사유가 아니다(#200). 증거가 없으면 중단·worktree 폐기(push 된 커밋은 재디스패치용으로 보존) |
| `STALL_MIN` | `25` | "무진전" 기준(분). 증거 = 브랜치 최신 커밋이 이보다 젊거나, 그 head SHA 의 CI 티켓이 박스 전역 직렬 큐에 살아 있음. `bin/ci` 한 판 + 큐 대기를 무진전으로 세지 않도록 잡은 값 |
| `MAX_TIMEBOX_GRACE` | `9` | 한 claim 안에서 허용하는 누적 유예 횟수 — 완화가 무한 유예가 되지 않게 하는 상한. 판정이 5분 틱에 이뤄지므로 실제 상한은 ~1시간 50분(타임박스 1시간 + 알아채는 데 최대 한 틱 + 유예 9틱 ≈ 45분). 틱 간격이 바뀌면 유예 창 ≈ 45분이 되도록 같은 비율로 맞춘다. 상태 파일이 아니라 append 전용 이슈 코멘트 마커로 센다 |
| `SOFT_TOKEN_BUDGET_PER_ISSUE` | `300k` | 관측치 전용 — 중단하지 않고 Report 에 승격 권고만 표시 |

그리고 숫자가 아닌 불변: **issue-runner 는 절대 머지하지 않는다.** `needs-human` 이슈는 **사람이 직접 세운 정지**다. 기계 정지는 `hold:*` 사유 라벨 하나만 단다(#244) — 게이트가 `hold:` **접두**를 보므로(#242) 그 라벨을 떼야 다시 흐르고, `hold:policy` 재심이 "사람 몫 유지" 로 끝난 건은 `needs-human` 까지 붙어 있으니 둘 다 떼야 한다. 그 기계 정지는 **이슈와 연결된 열린 PR 양쪽**에 미러되므로 해제도 **이슈와 PR 양쪽**에서 해야 한다 — 같은 네 게이트가 PR 라벨도 보기 때문에, 이슈만 깨끗해지고 PR 에 사본이 남으면 그 PR 은 어느 루프에도 안 잡힌다. 이슈에서만 뗐다면 다음 틱의 재개 스윕 정지 미러 정리(`scripts/resume-sweep.sh`, #265)가 PR 사본을 떼고, 그때까지는 `loop-status.sh` 가 `정지 미러 불일치` warn 으로 알린다. 그 정리는 라벨 **이벤트 이력**으로 "이슈에서 뗀 것이 PR 에 붙은 것보다 늦다" 를 확인했을 때만 돈다(전이가 PR 만 고치고 실패해 남은 반쯤 이동한 상태를 사람 해제로 오독하지 않게) — 그리고 **사람이 PR 에만 손으로 붙인 `needs-human`**(사유 라벨 없이 단독)은 기계가 만들 수 없는 모양이라 스윕이 **절대 떼지 않는다**. 머지 직전에 PR 한 건만 손으로 세우고 싶으면 그것이 제 통로다.
...
| `full-cycle` | **레인 소유** 표시 — 사람 세션 스킬 `full-cycle` 사이클이 들고 있는 산출물(구현 이슈·PR·배포 대기). 상태가 아니라 "누구 레인인가" 이며 `agent-ready` 와 **배타**(루프가 집지 않는다). `setup-labels.sh` 가 만든다(#245) — 없는 레포에선 `--add-label full-cycle` 편집이 통째로 실패한다 |
| `deploy-wait` | closeout 4단계·full-cycle §7 이 만든 배포 대기 이슈 — **deploy-cycle 루프의 레인**(사람 정지가 아니다). `loop-status.sh` 가 배포대기를 needs-human·보류와 가르는 버킷 라벨이기도 하다. closeout 은 `needs-human` 을 붙이지 않는다(#243) — 붙이는 주체는 **deploy-cycle** 이다(승격·배포·스모크 실패에, BoDAT #5197 — 숨은 정지 파일 폐지, 사람 몫은 이 라벨 하나). 그래서 2026-09-13 부터 버킷은 needs-human 이 배포대기보다 앞이다 |
| `테스트` | deploy-cycle ⑤ 가 배포 뒤에도 `- [ ]` 가 남은 배포 대기 이슈에서 항목을 그대로 옮겨 발행하는 배포 뒤 검증 이슈(BoDAT #5197) — 배포 대기 이슈는 닫힌다. 사람 정지가 아니라 **밟을 비용**(TEST 워커 런·시간 창·프로덕션 관측)이 남은 것 — 사용자가 `e2e-test` 스킬을 부를 때 비운다. 어느 루프도 안 집는다. `loop-status.sh` 버킷 `테스트`(needs-human 뒤·배포대기 앞). `needs-human` 과 축이 달라 겹칠 수 있다(실장비 **그리고** 사람 조작). `setup-labels.sh` 가 만든다 |
| `hold:conflict` · `hold:policy` · `hold:ladder` | **기계 정지**와 그 사유. `transition.sh verify-held|closeout-blocked|runner-held --reason <사유>` 가 **이 라벨을 단독으로** 붙인다(#244 — `needs-human` 을 겹쳐 붙이지 않는다. 그래야 needs-human 칸이 사람이 손댈 수 없는 것으로 차지 않는다). `ladder` 와 `conflict` 는 창(`RESUME_AFTER_MIN`)이 지나면 재개 스윕이 자동 재개하고(사람 개입 불요 — `ladder` 는 `LADDER_RESUME_LIMIT` 회, `conflict` 는 1회(`CONFLICT_RESUME_LIMIT`, #345)로 closeout 홀드 노트를 워커의 rebase 범위로 인라인한다; `full-cycle` 이 붙은 `hold:conflict` 는 사람이 인수한 것이라 절대 재개하지 않는다), 상한을 넘기면 둘 다 `policy` 로 승격되고 `policy` 는 루프 재심 1회(#155) 뒤 "사람 몫 유지" 판정 뒤에야 `needs-human` 이 붙는다(`transition.sh policy-kept`). **연결된 열린 이슈가 없는 PR** 의 `hold:conflict`(`closeout-blocked - <pr>`, 또는 홀드 뒤 참조 이슈 닫힘)는 재개할 워커가 없으므로 스윕의 PR 축이 창 뒤 `policy` 로 승격해 PR 단독 재심이 사람에게 잇는다(#345, #421). 그래서 `loop-status.sh`·`eligible-issues.sh` 는 단독 `hold:conflict` 를 **보류**(기계, 스스로 재개)로 세고, `needs-human` 과 `full-cycle` 인수만 사람 대기로 센다. 디스패치·검증·마감 게이트는 이 **접두**를 직접 본다(#242) — 사람이 홀드를 풀 때는 **이슈와 열린 PR 양쪽에서** 뗀다(PR 에 남은 사본은 스윕의 정지 미러 정리가 떼 준다, #265). `hold:dup`·`hold:hardware` 는 일부러 없다 — 중복은 `closeout-dup` 이 닫고, 실장비는 사다리를 오른다 |
...
**Codex 머지 게이트 = 내장 리뷰어.** `scripts/codex-review-gate.sh --base <ref> | --commit <sha> | --prompt "<text>"` 가 `codex exec review`(기본 `gpt-5.6-sol`, medium)를 감싸 발견을 판정으로 매핑한다 — `[P0]`·`[P1]` → BLOCKER(exit 1), `[P2]` → WARN, `[P3+]` → NIT, 없음 → CLEAN; 미산출(codex 부재·모델 오류·타임아웃) → exit 2 로 호출자가 fail-closed. 판정 입력은 **모델의 문장이 아니라 구조 신호**다(#375): ⑴ 렌더된 발견 항목(`- [P<n>]`)이 하나라도 있으면 모델이 무엇을 썼든 위 판정 ⑵ 항목 0 이고 정확히 `REVIEW_STATUS: no-basis` 인 줄이 있으면 exit 2 미산출(헬퍼가 `--prompt` 호출에 덧붙이는 선택 힌트 — 범위를 못 읽었으면 발견을 지어내지 말고 그 줄을 쓰라는 것) ⑶ 항목 0 이면 `--json` 이벤트 스트림(`<out>/events.jsonl`)에 `git ` 또는 `--cd` 경로가 든 **성공한** `command_execution` 이 하나 이상 있을 때만 CLEAN — diff 를 읽지 않은 "문제 없음" 은 CLEAN 이 아니라 exit 2 다. 옛 필수 마지막 줄 계약(`REVIEW_STATUS: reviewed`, #207/#283/#279)은 폐기했다 — 프로덕션 크기 프롬프트에서 모델이 그 줄을 내지 않았고(실호출 0/8) 게이트는 모델이 낸 판정을 버리고 있었다. `~/.codex/config.toml` 의 `model` 은 로그인 계정이 쓸 수 있는 모델로 유지할 것(`codex debug models`); 0.153+ 는 미설정 시 Astra 가 기본이다.

**실호출 스모크(선택, `bin/ci` 밖).** `scripts/manual/codex-review-gate-smoke.sh` 는 발견이 확실히 나는 diff 를 담은 임시 레포에 게이트를 **실제로** 1회 호출해 verdict 가 `NONE` 이 아님을 단언한다. `scripts/tests/` 의 스텁 격자는 이 부류의 결함을 구조적으로 못 본다 — 스텁은 발견 항목·이벤트 기록을 시키는 대로 내주므로, 진짜 CLI 가 그것을 **실제로 렌더하는지**는 스텁 밖의 사실이다. 네트워크와 codex 로그인이 필요해 CI 게이트에는 넣지 않는다; 게이트 판정부·프롬프트 계약문을 건드린 뒤, codex CLI 업그레이드 뒤, 계획부합 판정이 폴백으로만 날 때 손으로 돌린다. exit 0 = 통과, 1 = 회귀, 2 = 못 돌림(codex 부재), 3 = 미결정(발견이 안 렌더됨 — 무죄의 증거가 아니니 다시 돌려라).
