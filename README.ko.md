...
그래서 레포는 그 앞문으로 두 번째 스킬 **[`/loop-issues`](skills/loop-issues/SKILL.md)** 를 함께 담는다 — 무인으로 구현 가능한 이슈만 `agent-ready` 로 통과시키는 게이트다. 세 가지로 몬다(전부 자연어로 — "루프에 태워줘", "이 이슈 마감해줘", "이슈 트리아지해줘"):

- **이슈 하나 마감** — 지금 보는 이슈를 체크리스트에 통과시키고, **통과할 때만** `agent-ready` + 우선순위를 붙인다. 미달이면 잘못된 디스패치 대신 **뭐가 빠졌는지 목록**을 돌려준다.
- **큰 덩어리 스코핑** — 작업이 *high* 난이도면(여러 모듈을 건드리거나 합리적 설계 선택지가 둘 이상), 게이트가 **독립 검증 가능한 sub-issue 로 분해**(`Blocked by #N` 로 직렬화)하거나 **`## Plan`**(각 단계가 건드리는 파일 + 단계별 검증 명령을 담은 번호 task 목록)을 첨부하게 만든다. 이 **계획→이슈** 단계가, 큰 아이디어를 하나의 거대·모호한 티켓이 아니라 루프에 태울 수 있는 일로 바꾸는 지점이다.
- **백로그 일괄 트리아지** — 레포(또는 계정 전체)를 겨냥하면 열린 이슈를 전부 **READY / FIXABLE / UNFIT** 로 분류하고, 고칠 수 있는 건 보완안을 제안한다. 승인 전에는 아무것도 재라벨하지 않는다.

게이트가 강제하는 체크리스트, 요약하면: **자기완결 스펙** · **체크박스 수용 기준**("잘 동작" 금지) · 실제 명령이 든 **`## Test plan`** · `Blocked by #N` 라인의 **의존성** · **우선순위** 라벨 · **epic → 분해, leaf 에만 라벨** · 그리고 위 **난이도** 판단. `agent-ready` 는 이 모든 걸 마친 뒤 **맨 마지막**에 붙인다 — 디스패처는 임의로 붙이지 않는다.
