...
issue-runner 는 자체 바이너리·데몬·세션 저장소를 가진 독립 러너가 *일부러* 아니다. Claude Code 를 위한 얇은 **스킬**이고, 그 선택이 강점을 규정한다:
...
한 작업을 깊게 모는 풍부한 하네스가 필요하면 러너를 써라. 자리를 비운 사이 `agent-ready` 백로그가 조용히 리뷰 가능한 PR 로 바뀌길 원하면, 그게 이것이다.
