...
그리고 숫자가 아닌 불변: **issue-runner 는 절대 머지하지 않는다.** `needs-human` 이슈는 **사람이 직접 세운 정지**다. 기계 정지는 `hold:*` 사유 라벨 하나만 단다(#244) — 게이트가 `hold:` **접두**를 보므로(#242) 그 라벨을 떼야 다시 흐르고, `hold:policy` 재심이 "사람 몫 유지" 로 끝난 건은 `needs-human` 까지 붙어 있으니 둘 다 떼야 한다.
