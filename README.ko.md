...
그리고 숫자가 아닌 불변: **issue-runner 는 절대 머지하지 않는다.** `needs-human` 이슈는 루프가 손을 뗀 상태 — 사람이 원인을 보고 라벨을 제거해야 다시 흐른다. 게이트는 `hold:` **접두** 라벨도 보므로(#242) 기계 정지(`hold:conflict`·`hold:policy`·`hold:ladder`)를 풀 때는 `needs-human` 과 `hold:*` 를 **둘 다** 떼야 한다 — 한쪽만 떼면 후보로 돌아오지 않는다. 기계 정지는 **이슈와 연결된 열린 PR 양쪽**에 미러되므로 해제도 **이슈와 PR 양쪽**에서 해야 한다 — 같은 네 게이트가 PR 라벨도 보기 때문에, 이슈만 깨끗해지고 PR 에 사본이 남으면 그 PR 은 어느 루프에도 안 잡힌다. 이슈에서만 뗐다면 다음 틱의 재개 스윕 정지 미러 정리(`scripts/resume-sweep.sh`, #265)가 PR 사본을 떼고, 그때까지는 `loop-status.sh` 가 `정지 미러 불일치` warn 으로 알린다. 그 정리는 라벨 **이벤트 이력**으로 "이슈에서 뗀 것이 PR 에 붙은 것보다 늦다" 를 확인했을 때만 돈다(전이가 PR 만 고치고 실패해 남은 반쯤 이동한 상태를 사람 해제로 오독하지 않게) — 그리고 **사람이 PR 에만 손으로 붙인 `needs-human`**(사유 라벨 없이 단독)은 기계가 만들 수 없는 모양이라 스윕이 **절대 떼지 않는다**. 머지 직전에 PR 한 건만 손으로 세우고 싶으면 그것이 제 통로다.
...
| `hold:conflict` · `hold:policy` · `hold:ladder` | `needs-human` 의 **사유**. `transition.sh verify-held|closeout-blocked --reason <사유>` 가 함께 붙인다(사유 없는 `needs-human` 은 만들 수 없다). `ladder` 만 재개 스윕이 자동 재개한다. 디스패치·검증·마감 게이트는 이 **접두**를 직접 본다(#242) — 사람이 홀드를 풀 때는 `needs-human` 과 함께, **이슈와 열린 PR 양쪽에서** 뗀다(PR 에 남은 사본은 스윕의 정지 미러 정리가 떼 준다, #265). `hold:dup`·`hold:hardware` 는 일부러 없다 — 중복은 `closeout-dup` 이 닫고, 실장비는 사다리를 오른다 |
