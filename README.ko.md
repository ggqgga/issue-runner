...
| `deploy-wait` | closeout 4단계·full-cycle §7 이 만든 배포 대기 이슈 — **deploy-cycle 루프의 레인**(사람 정지가 아니다). `loop-status.sh` 가 배포대기를 needs-human·보류와 가르는 버킷 라벨이기도 하다. `needs-human` 은 붙이지 않는다(#243) — 디스패치 게이트는 `agent-ready` 를 요구하고, 버킷은 `deploy-wait` 가 이기며, deploy-bodat 수집은 제목 정규식이라 그 라벨을 아무도 안 본다 |
