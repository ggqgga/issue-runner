...
- `warn` — 사유 라벨(`hold:*`) 없는 `needs-human`(사람이 손으로 붙였을 수 있어 자동 재개
  대상이 아니다) · 사람 조작과의 경합 · 편집 **전** 실패. 스크립트가 **손대지 않은** 건이다 —
  **건드리지 말고** ④ Report 의 warn 에 그대로 옮겨라.
- `warn_after_edit` — 라벨은 **이미 반영된 뒤**의 부수 실패(코멘트 실패 · 승격/재개 readback 조회
  실패·불일치 · 라벨 해제 실패). 재개/승격 자체는 일어났을 수 있으니 되돌리지 말고, ④ Report 의
