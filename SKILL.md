...
- `MAX_TIMEBOX_GRACE = 9` — 같은 claim 에서 허용하는 **누적 유예 횟수**(사이에 `unknown`
  틱이 끼어도 리셋되지 않는다 — 세는 창은 claim 시각 이후 전부다). 넘으면 진행
  증거가 있어도 규칙대로 중단한다 — 유예가 무한이면 진짜 좀비를 못 잡아 이 완화 자체가
  새 구멍이 된다. 틱 간격 5분 기준 최대 ~45분의 추가 시간이라 실측(72분·64분) 형상을
  덮으면서도 상한이 남는다. **2026-09-13 에 3→9 로 상향** — 크론이 15분에서 5분
  주기로 바뀌어 유예 3회가 15분(설계 45분의 1/3)으로 줄었고, PR 직전 완성 상태의
  워커를 끊은 실측(bodat #4977: 중단 후 재디스패치가 코드 변경 0 으로 PR 만 열었다)이
  나왔다. 틱 간격을 다시 바꾸면 이 값도 같은 비율로 맞춰라(유예 창 ≈ 45분). 횟수는
  상태 파일이 아니라 이슈 코멘트 마커 (`<!-- timebox-grace: N -->`)를 **현재 claim 시각 이후 것만** 세어 재파생한다.
