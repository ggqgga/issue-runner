...
- `policy_review_due` — `hold:policy` 로 멈춘 지 `RESUME_AFTER_MIN` 이 지났는데 아직 재심을 안 한
  ...
  **연결 PR 이 있으면(`pr` ≠ `-`) 그 코멘트를 PR 에도 미러하라**(#174 재작업) — closeout 의
  `pr-hold-released-at.sh` 가 PR 타임라인의 `hold:policy` 해제를 보고 "이건 사람 해제가 아니라
  이 재심 마커가 붙인 기계 해제다" 를 판정하는 근거가 이 마커다. 이슈에만 남기면 그 판정이
  못 보고 새서, 가려진 낡은 `머지 판정: ✅` 가 검증 없이 되살아날 수 있다(closeout SKILL
  ③-1 3항 참조).
