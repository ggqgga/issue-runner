...
초안 2026-09-13 (사람 결정 대기) · 시각화: `loop-restructure.html` · 진단 원본: 탐색 에이전트 2건(스킬 문서 축 · 스크립트/CI 축)
...
4. **가드는 대체 후 폐기.** bin/ci 의 문자열 단언은 그 규칙이 행동 테스트나 SSOT 로 강제되는 순간 지운다.
   무조건 삭제도, 영구 보존도 아니다.
5. **매 틱 로드 크기 목표**: closeout ≤ 400줄 · issue-runner ≤ 300줄 · verify-runner ≤ 250줄.
...
| 6 | rationale 한/영 | 한글만 | 매 틱 로드되지 않는 문서에 번역 동기 비용을 안 들임 — **사람 결정 필요** |
| 7 | 에픽 수 | 5개(단계 = 에픽), P1 은 0·1 부터 | 동시 P1 에픽 2개 규약 |
| 8 | closeout 목표 크기 | ≤ 400줄 | 진단상 운영 지시 실분량(약 1/3)에 여유를 둔 값 |
...
1. 이 초안에 대한 사람 피드백(특히 결정 로그 #1·#5·#6).
2. 합의되면 docs PR 로 main 에 머지 → 0단계 에픽 + leaf 4개 발행 → full-cycle 로 `state-machine.md`.
