...
**꼬리 펜스는 짝으로 가른다(#279 — "맨몸 펜스를 여닫이 구분 없이 벗긴다" 잔여를 닫았다).** 맨몸 펜스(``` · ~~~)는 문자열만으로
여닫이가 안 갈리므로, 벗기기 전에 본문 처음부터 CommonMark 규칙(들여쓰기 ≤3칸 · 닫는 펜스엔
정보 문자열 없음 · 여는 펜스와 같은 문자·같은 길이 이상 · 백틱 여는 펜스의 정보 문자열엔 백틱
금지)으로 펜스를 세어 그 꼬리 펜스가 **앞에서 열린 블록을 닫는 줄**인지 판정하고 짝이 맞는 것만
벗긴다(awk 한 패스, 새 의존 없음). 짝이 없으면 여는 펜스이므로 안 벗기고 → 그 줄이 마지막 줄이
되어 계약 위반 → 미산출(fail-closed). 방향은 **과잉 차단 0** 이 우선이다 — 리뷰 전체를 펜스로
