...
**Codex merge gate = the built-in reviewer.** `scripts/codex-review-gate.sh --base <ref> | --commit <sha> | --prompt "<text>"` wraps `codex exec review` (default `gpt-5.6-sol`, medium) and maps its findings to a verdict — `[P0]`/`[P1]` → BLOCKER (exit 1), `[P2]` → WARN, `[P3+]` → NIT, none → CLEAN; no verdict (codex missing, model error, timeout) → exit 2 so callers fail closed. With `--prompt` the helper also appends a response contract (the review's last line must carry a fixed status token) and decides on that line alone — missing/broken line or a `no-basis` status → exit 2, no verdict (#207: prose "I had no basis to judge" wording used to be read as CLEAN, a fail-open merge gate). Keep `model` in `~/.codex/config.toml` set to a model your sign-in can use (`codex debug models`); 0.153+ defaults to Astra when unset.
