...
**Hitting the verify attempt limit reissues (#301).** When the same PR has been bounced `VERIFY_ATTEMPTS_LIMIT` (3) times, verify-runner does not ask a human — it **reissues** via `scripts/reissue-pr.sh`: it files a fresh `agent-ready` issue carrying the verifier's last BLOCKER/WARN verbatim (machine comments only — a human quoting them is not a candidate), the current round's failure reason (`--reason`) and the original acceptance criteria (labels, the `Epic #N` line and the priority are inherited; `spinoff` is deliberately **not** attached — a reissue is a retry, not a spinoff), closes the PR (keeping the branch), and closes the original issue as `not planned`. The write order is the contract: **file → confirm → close the original issue → close PR** — reversed, a failure closes the original while no new issue exists (if filing fails, nothing is closed). The PR goes last so the helper can re-enter: if closing the original fails, the PR stays open in the verify queue and the next tick comes back through the same PR (idempotent via the `<!-- reissued: #N -->` marker) to finish the remaining closes. The evidence is from 2026-09-11: all four PRs that hit the limit were granted one more round by a human and three of them ended that round on a BLOCKER too — a fresh worker handed the verifier's final spec beats a worker inheriting a pile of bounce context. **Only a human grants an exception round, and there is exactly one phrase for it** — a comment on the original issue carrying `회차 허용: +1 — 범위: <one line>` ("one more round — scope: …") — at the start of a line or after a lead-in such as `사람 결정: **ⓐ — `, the exact shape humans wrote on 2026-09-12; the scope runs to the end of that line with bold markers stripped; ① Reconcile reads it, rolls `verify-attempt` back to `LIMIT-1`, and carries that scope sentence verbatim into the bounce prompt (idempotent through a `<!-- round-granted -->` marker — **one exception per issue**, a second phrase is ignored with a warn; quoted phrases (code fences/backticks) and machine comments are not signals). If that round hits the limit too, it reissues. So `hold:policy` no longer carries the "grant one more round?" question.
...
| `deploy-wait` | A deploy-pending issue created by closeout step 4 / full-cycle §7 — **the deploy-cycle loop's lane** (not a human stop). Also the bucket label `loop-status.sh` uses to separate deploy-waiting from human-waiting/held. `needs-human` is not attached (#243) — the dispatch gate requires `agent-ready`, the bucket lets `deploy-wait` win, and deploy-bodat collects by title regex, so nobody reads that label here |
| `hold:conflict` · `hold:policy` · `hold:ladder` | A **machine stop** and its reason. `hold:policy` is for a **spec/policy choice only** — hitting the verify redispatch limit is not held any more, it is reissued by `scripts/reissue-pr.sh` (#301; the one human exception is a `회차 허용: +1 — 범위: <one line>` comment on the issue). Attached **alone** by `transition.sh verify-held|closeout-blocked|runner-held --reason <reason>` (#244: `needs-human` is no longer attached alongside, so the human-waiting bucket stops filling with things a human cannot act on). Only `ladder` is auto-resumed by the resume sweep; `policy` gets one loop re-review (#155) and only a "kept" verdict attaches `needs-human` (`transition.sh policy-kept`); `conflict` is a human's job on its own. The dispatch/verify/closeout gates read this **prefix** directly (#242) — when a human clears a hold, remove it **on the issue and on its open PR** (the sweep's stop-mirror cleanup drops a leftover PR copy, #265). `hold:dup` and `hold:hardware` deliberately do not exist — duplicates are closed by `closeout-dup`, and hardware climbs the ladder |
...
**Codex merge gate = the built-in reviewer.** `scripts/codex-review-gate.sh --base <ref> | --commit <sha> | --prompt "<text>"` wraps `codex exec review` (default `gpt-5.6-sol`, medium) and maps its findings to a verdict — `[P0]`/`[P1]` → BLOCKER (exit 1), `[P2]` → WARN, `[P3+]` → NIT, none → CLEAN; no verdict (codex missing, model error, timeout) → exit 2 so callers fail closed. With `--prompt` the helper also appends a response contract (the review's last line must carry a fixed status token) and decides on that line alone — missing/broken line or a `no-basis` status → exit 2, no verdict (#207: prose "I had no basis to judge" wording used to be read as CLEAN, a fail-open merge gate). That "last line" means the last line of the **model-authored region**, not of the file: when a review has findings, `codex exec review` appends its own section header and renders the item list after the model's summary, so the file's last line is codex's, never the model's (#283 — asking the file made every finding-bearing review return no verdict, i.e. the gate discarded exactly the BLOCKERs and WARNs while letting CLEANs through). The **whole line** must be that format: trailing text after the value breaks it, and so does any text before the value on the same line, whatever it says (#283 — two rounds tried to tell prefixes apart by wording and let *"I could not inspect the diff. &lt;status line&gt;"* through as CLEAN; enumerating wordings never closes). A status line appended to the end of a summary sentence is therefore not read as a verdict, which is why the contract text spells out "put no other text on that line (before or after it)". The codex render section that ends the model-authored region is likewise recognized only when the **CLI** wrote it: an item must follow the header, everything from there **to end of file** must be an item line or an item's indented body, and exactly **one** such header may exist (codex appends a single section after the model's message, so the render is the document's suffix). If the model fabricates a header and a `- [Pn]` bullet but prose still follows them, that is not a boundary; if two headers qualify, the boundary is ambiguous and nothing is cut — either way the call falls through to no verdict (fallback). Keep `model` in `~/.codex/config.toml` set to a model your sign-in can use (`codex debug models`); 0.153+ defaults to Astra when unset.

**Real-call smoke (optional, outside `bin/ci`).** `scripts/codex-review-gate-smoke.sh` calls the gate once for real against a throwaway repo whose diff is sure to draw a finding, and asserts the verdict is not `NONE`. The stubbed grid in `scripts/tests/` cannot see this class of bug — a stub emits the contract line on demand, so whether the real CLI *can* emit it is a fact outside the stub. Needs network and a codex sign-in, so it stays out of the CI gate; run it by hand after touching the gate's verdict logic or prompt contract, after a codex CLI upgrade, or whenever plan-conformance verdicts start coming only from the fallback. Exit 0 = pass, 1 = regression, 2 = cannot run (no codex), 3 = undecided (no findings rendered — not proof of innocence, run it again).
