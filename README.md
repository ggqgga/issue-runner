...
| `deploy-wait` | A deploy-pending issue created by closeout step 4 / full-cycle §7 — **the deploy-cycle loop's lane** (not a human stop). Also the bucket label `loop-status.sh` uses to separate deploy-waiting from human-waiting. `needs-human` is not attached (#243) — the dispatch gate requires `agent-ready`, the bucket lets `deploy-wait` win, and deploy-bodat collects by title regex, so nobody reads that label here |
| `hold:conflict` · `hold:policy` · `hold:ladder` | The **reason** behind `needs-human`. Attached together by `transition.sh verify-held|closeout-blocked --reason <reason>` (a reason-less `needs-human` cannot be produced). Only `ladder` is auto-resumed by the resume sweep. The dispatch/verify/closeout gates read this **prefix** directly (#242) — when a human clears a hold, remove it alongside `needs-human`, **on the issue and on its open PR** (the sweep's stop-mirror cleanup drops a leftover PR copy, #265). `hold:dup` and `hold:hardware` deliberately do not exist — duplicates are closed by `closeout-dup`, and hardware climbs the ladder |
