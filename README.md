...
**Codex merge gate = the built-in reviewer.** `scripts/codex-review-gate.sh --base <ref> | --commit <sha> | --prompt "<text>"` wraps `codex exec review` (default `gpt-5.6-sol`, medium) and maps its findings to a verdict — `[P0]`/`[P1]` → BLOCKER (exit 1), `[P2]` → WARN, `[P3+]` → NIT, none → CLEAN; no verdict (codex missing, model error, timeout) → exit 2 so callers fail closed. Keep `model` in `~/.codex/config.toml` set to a model your sign-in can use (`codex debug models`); 0.153+ defaults to Astra when unset.
