...
| `deploy-wait` | A deploy-pending issue created by closeout step 4 / full-cycle §7 — **the deploy-cycle loop's lane** (not a human stop). Also the bucket label `loop-status.sh` uses to separate deploy-waiting from needs-human/held. `needs-human` is not attached (#243) — the dispatch gate requires `agent-ready`, the bucket lets `deploy-wait` win, and deploy-bodat collects by title regex, so nobody reads that label here |
