...
So the repo ships a second skill, **[`/loop-issues`](skills/loop-issues/SKILL.md)**, as that front door — a gate that only lets an issue become `agent-ready` once it can actually be built unattended. Three ways to drive it (all by natural language — "put this on the loop", "close this issue out", "triage my issues"):

- **Close out one issue** — take the issue you're looking at, run it through the checklist, and attach `agent-ready` + a priority **only if it passes**. If it doesn't, you get a list of what's missing instead of a bad dispatch.
- **Scope a larger piece** — when work is *high* difficulty (touches several modules, or has more than one reasonable design), the gate makes you either split it into independently-verifiable **sub-issues** (serialized with `Blocked by #N`) or attach a **`## Plan`** — a numbered task list with the files each step touches and a verify command per step. That plan-to-issues step is how a big idea becomes loop-ready work instead of one giant ambiguous ticket.
- **Batch-triage a backlog** — point it at a repo (or your whole account) and it classifies every open issue as **READY / FIXABLE / UNFIT**, proposing fixes for the fixable ones. Nothing is relabeled until you approve.

The checklist it enforces, in short: a **self-contained spec** · **checkbox acceptance criteria** (no "works well") · a **`## Test plan`** with real commands · **dependencies** as `Blocked by #N` lines · a **priority** label · **epic → split, label only leaves** · and the **difficulty** judgment above. Attach `agent-ready` **last**, after all of it — the dispatcher never adds it on its own.
